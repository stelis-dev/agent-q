...
Provider-facing signing activation uses these status terms:
...
Current `request_signature` status is `provider-exposed-not-product-active` with current-tree target
positive/reject/timeout/session-loss hardware smoke recorded under `.WORK`.
LVGL visual evidence is still pending, so do not claim `product-active`.
...
| `get_capabilities` | O | StackChan CoreS3 source reports Firmware-authored Sui Ed25519 account identity capability over an approved session while `provisioned`. Delegated public method capabilities are empty. Provider-facing signing availability is advertised through top-level `signatureRequests` for the `provider-exposed-not-product-active` `request_signature` path; it is not advertised through `chains[].methods` and MCP must fail closed rather than expose provider-facing signing metadata as an agent-facing capability. |
...
| `get_approval_history` | △ | Read-only session-scoped approval-history path exists in Gateway/MCP and StackChan CoreS3 source. Firmware stores bounded persistent method-decision, policy-update terminal, and current-schema signature-request confirmation/terminal metadata in a fixed-size NVS ring buffer. Product-reachable method-decision records cover validated policy-rejected `call_method` decisions; invalid parameter, malformed transaction, and unsupported-method errors are not persisted as approval history. The `propose_policy_update` flow emits recordable policy-update terminal records through a required-write path that is separate from method-decision write budgeting. `provider-exposed-not-product-active` `request_signature` creates pre-signing confirmation and terminal signature-request records. Current-tree provider positive/reject/timeout/session-loss hardware smoke is recorded; LVGL visual evidence remains pending before `product-active`. History stores no raw txBytes, decoded transaction, session id, request id, gateway name, PIN, secret material, raw policy, or full policy document. Local reset and error-state erase recovery wipe it. Gateway/MCP parser tests, the target approval-history host test, and opt-in policy-update hardware smoke coverage cover current product-reachable history behavior. |
| `call_method` | △ | Gateway and StackChan CoreS3 source support the common provisioned/session-scoped method path. Unknown methods are rejected with `unsupported_method`. Sui `sign_transaction` accepts only bounded restricted SUI transfer request inputs for validation and policy evaluation, consumes the committed active policy, persists required policy-rejected approval-history metadata, rejects unsupported transactions, and returns rejected method results. Signing output through `call_method` is not implemented. Corrupt/unreadable, missing, or invalid current policy material fails closed as a material-consistency error before normal session-scoped methods are available. Host tests cover request field/type validation, policy-store provider behavior, and method runtime rejected paths. |
| `request_signature` | △ | StackChan CoreS3 source has `provider-exposed-not-product-active` status for provider-facing device-confirmed Sui `sign_transaction` over the bounded restricted-transfer shape. The public USB dispatcher, `signature_result` writer, Gateway core/client request builder and parser, provider `requestSignature` API, and provider-facing `signatureRequests` capability are present. Firmware owns material/session/busy gates, `txBytes`-derived review facts, host-supplied `network` validation without displaying it as a transaction-derived fact, local PIN confirmation, required pre-signing history, signing-critical handoff, terminal history, response delivery, and cleanup. It is not an MCP signing tool, does not use `call_method`, and does not accept caller-controlled timing fields. Firmware-owned review/PIN input windows are modeled as a fixed internal 30-second window; stored-PIN cryptographic verification is not timed out by the local input window, the internal local-auth worker watchdog still fails closed as authentication unavailable, and wrong PIN results open a fresh 30-second input window only within the original fixed confirmation deadline unless the shared lockout is active. Current-tree hardware smoke passed for provider positive signed result, device reject, review timeout, and USB session loss; LVGL visual evidence remains pending, so current status is not yet `product-active`. |
...
| Signing APIs | △ | Provider-facing signing has `provider-exposed-not-product-active` status through Gateway core, the USB driver, the public protocol parser, and the provider package as `requestSignature` / `request_signature` / `signature_result` with provider-facing `signatureRequests`. MCP has no signing tool and its get-capabilities runtime and exported output schema reject provider-facing signing metadata. Provider exposes signing transport only and still does not expose raw `callMethod` or Admin/policy update entrypoints. Current-tree target positive/reject/timeout/session-loss hardware smoke is recorded; LVGL visual evidence remains pending, so this is not yet `product-active`. |
...
| `get_capabilities` | O | Gateway parser accepts Firmware-authored Sui Ed25519 account identity capability over an active runtime session with `chains[].methods: []` and provider-facing `signatureRequests` for provider use. MCP never exposes the session id, does not expose a signing tool, and fails closed if provider-facing `signatureRequests` reaches the MCP output boundary. |
...
| `get_approval_history` | △ | Gateway tool and protocol parser exist for read-only persistent Firmware approval history over an active runtime session. The parser accepts bounded method-decision records, policy-update terminal records, and current-schema `signature_request` confirmation/terminal records, preserves sequence and uptime as strings, rejects session ids and secret-like fields, and MCP never exposes the session id. `signature_request` history parsing is used by the `provider-exposed-not-product-active` provider signing path; current-tree provider positive/reject/timeout/session-loss hardware smoke is recorded, with LVGL visual evidence pending. Opt-in policy-update hardware smoke coverage exists for newest policy-update terminal history over MCP. |
| `call_method` | △ | Gateway tool and protocol parser exist for the common session-scoped method path. Current public method results are rejected-only; approved signing through `call_method` is not accepted by the client/MCP schema. Device-confirmed signing is exposed only through provider-facing `request_signature`, not MCP, and remains below `product-active`. |
...
| `get_capabilities` | O | X | X | StackChan CoreS3 source reports Sui Ed25519 account identity capability for account 0 over an approved session while `provisioned`. Delegated public method capabilities are empty. Provider-facing signing availability is advertised through top-level `signatureRequests` for the `provider-exposed-not-product-active` `request_signature` path; MCP rejects that provider-facing metadata. |
...
| `get_approval_history` | △ | X | X | StackChan CoreS3 source stores bounded persistent method-decision, policy-update terminal, and current-schema signature-request confirmation/terminal metadata in a fixed-size NVS ring buffer and exposes it through a read-only session-scoped protocol request. Product-reachable method-decision records cover validated policy-rejected decisions from `call_method`. The policy-update flow records terminal metadata from `propose_policy_update`; the `provider-exposed-not-product-active` `request_signature` path creates signature-request confirmation and terminal records. Current-tree provider positive/reject/timeout/session-loss hardware smoke is recorded; LVGL visual evidence remains pending before `product-active`. Gateway/MCP parser tests, a target approval-history host test, and opt-in policy-update hardware smoke coverage cover current product-reachable history behavior. |
| `call_method` | △ | X | X | StackChan CoreS3 source enforces provisioned/session gates, keeps unknown methods rejected with `unsupported_method`, and validates Sui `sign_transaction` restricted SUI transfer request inputs for rejected-path policy evaluation. It consumes the committed active policy, rejects unsupported transactions, and returns rejected method results. Corrupt/unreadable, missing, or invalid current policy material fails closed as material consistency before normal session-scoped methods are available. Signing output through `call_method` is not implemented. Host tests cover the Firmware request field/type validation boundary, policy-store provider behavior, and method runtime rejected paths. |
| `request_signature` | △ | X | X | StackChan CoreS3 source has `provider-exposed-not-product-active` status for provider-facing device-confirmed Sui `sign_transaction` over the bounded restricted-transfer shape. The public USB dispatcher, `signature_result` writer, provider/client parser/API, and provider-facing `signatureRequests` capability are present. Host tests cover material/session/busy gating, `txBytes`-derived review facts, local PIN, required confirmation history, signing-critical handoff, terminal metadata, response boundaries, and cleanup behavior. Current-tree target positive/reject/timeout/session-loss hardware smoke is recorded; LVGL visual evidence remains pending, so this is not yet `product-active`. |
...
| Per-request signing approval | △ | N/A | X | StackChan CoreS3 source has provider-facing device-confirmed signing approval for `request_signature` with local PIN and required pre-signing history under `provider-exposed-not-product-active` status. Current-tree target positive/reject/timeout/session-loss hardware smoke is recorded; LVGL visual evidence remains pending, so product-active status is not yet claimed. |
...
| Persistent signing material | △ | X | X | StackChan CoreS3 source stores DEV_PROFILE BIP-39 root entropy after backup confirmation plus matching local PIN repeat. Public account derivation is implemented (`get_accounts`, Sui Ed25519 account 0). Internal Sui transaction signing substrate is wired into provider-facing `request_signature` for the bounded restricted transfer shape. Current-tree provider positive/reject/timeout/session-loss hardware smoke is recorded; LVGL visual evidence remains pending. USER_PROFILE secure storage and import are not implemented. |
...
|---|---:|---:|---:|---:|---|
| StackChan CoreS3 | △ | X | X | X | Boot-time Sui Ed25519 self-test, read-only `get_accounts` account-0 derivation, and `provider-exposed-not-product-active` `request_signature` for the bounded restricted-transfer shape. Current-tree provider positive/reject/timeout/session-loss hardware smoke is recorded; LVGL visual evidence remains pending. Broader Sui transaction shapes and other chains are not implemented. |
...
| Sui `sign_transaction` | △ | Through `call_method`, this method validates `network` and base64 `txBytes`, decodes the restricted SUI transfer shape, consumes the committed active policy runtime decision, rejects unsupported transactions, and returns rejected method results. Provider-facing `request_signature` has `provider-exposed-not-product-active` status for the same bounded restricted-transfer shape with device confirmation, required history, signing, and `signature_result`. Current-tree provider positive/reject/timeout/session-loss hardware smoke is recorded; LVGL visual evidence remains pending. Corrupt/unreadable, missing, or invalid current policy material fails closed as material consistency before normal session-scoped methods are available. |
