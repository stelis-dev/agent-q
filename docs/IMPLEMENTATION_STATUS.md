...
| Signing APIs | X | No signing API exists. A future signing method must be exposed through common `call_method`, not chain-specific top-level MCP tools, and must update capability advertisement, Gateway parser/output schemas, MCP output, provider API, Firmware method-approval state, and Firmware approval-history handling together before it is presented as available. |
...
| Sui `sign_transaction` | △ | This method is recognized inside `call_method` only for policy-decision smoke. It validates `network` and base64 `txBytes`, decodes the restricted SUI transfer shape, consumes the committed active policy runtime decision, and returns a rejected `method_result`; corrupt/unreadable, missing, or unsupported current policy material fails closed as material consistency before normal session-scoped methods are available. It is not advertised in `get_capabilities`, does not sign, and does not trigger approval UI. Hardware smoke coverage exists for this stored-policy reject path. Moving it to signing support requires capability advertisement, approved/signature result parsing, a Firmware-owned method approval state, policy `ask`/`sign` handling, required approval-history behavior, MCP output, and provider API changes in the same implementation boundary. |
