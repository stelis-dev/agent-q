...
pre-cutover wire names only. The current Sign API wire names still require
current-tree flash, hardware smoke for the implemented authorization outcomes,
...
| Persistent signing material | △ | X | X | StackChan CoreS3 source stores DEV_PROFILE BIP-39 root entropy after backup confirmation plus matching local PIN repeat. Public account derivation is implemented (`get_accounts`, Sui Ed25519 account 0). Internal Sui signing substrate is wired into `sign_transaction` for the bounded restricted transfer shape and into user-mode `sign_personal_message` for bounded personal-message bytes. Current-tree hardware smoke remains pending after the Sign API cutover. USER_PROFILE secure storage and import are not implemented. |
