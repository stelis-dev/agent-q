...
| `get_approval_history` | △ | Read-only session-scoped approval-history path exists in Gateway/MCP and StackChan CoreS3 source. Firmware stores bounded persistent method-decision metadata in a fixed-size NVS ring buffer, currently only for validated `call_method` policy-rejected decisions. The storage/read schema and Gateway/MCP parser also accept future `policy_update` terminal records, but no current protocol path emits them. Invalid parameter, malformed transaction, and unsupported-method errors are not persisted as approval history. Method-decision writes are Firmware-rate-limited to reduce flash wear; required future policy-update terminal records use a separate required-write path. It stores no raw txBytes, decoded transaction, session id, request id, gateway name, PIN, secret material, raw policy, or full policy document. Local reset and error-state erase recovery wipe it. Gateway/MCP parser tests and the target approval-history host test cover this path; hardware smoke remains required. |
...
| `get_approval_history` | △ | Gateway tool and protocol parser exist for read-only persistent Firmware approval history over an active runtime session. The parser accepts bounded method-decision records and future policy-update terminal records, preserves sequence and uptime as strings, rejects session ids and secret-like fields, and MCP never exposes the session id. Hardware smoke is still required. |
...
| Policy storage/read | △ | X | X | StackChan CoreS3 stores the active policy as a canonical `agentq.policy.v0` binary record in two bounded NVS slots plus commit metadata and a pending-write marker. Backup confirmation plus matching local PIN repeat still installs only the DEV_PROFILE default-reject policy, but the storage/provider boundary can load canonical custom reject-policy records. It exposes a read-only `get_policy` summary, preserves the previous committed policy only for interrupted writes identified by the pending marker, treats metadata flip as the commit point, classifies each write as applied, unchanged failure, or consistency error, tolerates stale pending markers that exactly match the selected committed policy, removes stale commit metadata before slot reuse, and treats corrupt/unreadable committed policy, invalid commit metadata without that marker, or pending targets that overlap active material without exactly matching it as a material-consistency error. Policy update authorization is not implemented. |
...
| Policy storage/read | △ | StackChan CoreS3 stores the active policy as a canonical `agentq.policy.v0` binary record in NVS. The target store supports two bounded slots plus commit metadata and a pending-write marker, preserves the previous committed policy only for interrupted inactive-slot or metadata writes identified by that marker, treats metadata flip as the commit point, classifies each write as applied, unchanged failure, or consistency error, tolerates stale pending markers that exactly match the selected committed policy, removes stale commit metadata before slot reuse, exposes a read-only `get_policy` summary for the committed active policy, and treats corrupt/unreadable active policy, invalid commit metadata without a matching pending marker, or overlapping pending targets that do not exactly match the selected active material as a material-consistency error. The current product flow still installs only the DEV_PROFILE default-reject policy; protocol policy update authorization and Admin/Gateway policy proposal submission are not implemented. |
| Approval history storage/read | △ | StackChan CoreS3 stores bounded persistent approval-history records in NVS and exposes a read-only session-scoped `get_approval_history` view. The current runtime records only validated policy-rejected metadata from the `call_method` skeleton, rejects unsupported current-schema storage blobs, uses Firmware-owned method-decision write rate limiting, and returns fail-closed `history_error` when a required record cannot be persisted. The schema includes future `policy_update` terminal records through a required-write path that is not consumed from the method-decision budget; future user approval, method-error, and signing decisions are not implemented. Reset and error-state erase wipe the history. |
| Policy update authorization | X | Not implemented. The design contract is now specified as a future Firmware-owned `propose_policy_update` flow: Gateway/Admin may submit a bounded policy proposal, Firmware validates it, requires device-local approval, commits through the target's canonical active-policy store, records a required policy-update terminal history record, and rejects direct setters such as `set_policy` or `force_policy`. StackChan CoreS3 has a target-local JSON policy-proposal parser/validator source, a two-slot canonical active-policy store foundation, and a small persistent terminal marker that makes an incomplete post-commit policy-update terminal sequence a material-consistency error on reboot, but no raw protocol envelope handler, Gateway/Admin surface, Firmware pending-update state, policy proposal commit flow, or approval UI exists yet. |
