...
| Factory reset / material wipe | △ | StackChan CoreS3 source adds a physical-approval `factory_reset` path that clears RAM sessions, wipes volatile setup scratch, erases the DEV_PROFILE root entropy blob and active policy, persists `unprovisioned`, and recovers from material/state consistency error. This is a DEV_PROFILE development/recovery path and is not exposed as a normal agent-facing MCP tool. Hardware smoke is still required. |
| `identify_device` | O | Implemented as temporary device UI for explicit user selection. |
| `connect` | O | Protocol and Gateway parser support exist. StackChan CoreS3 hardware smoke verifies provisioned-only Firmware session creation after physical approval. |
| `disconnect` | O | Protocol and Gateway parser support exist. StackChan CoreS3 hardware smoke verifies clearing a matching active Firmware session after `provisioned`. |
| `get_capabilities` | O | StackChan CoreS3 hardware smoke verifies Firmware-authored Sui Ed25519 account identity capability over an approved session while `provisioned`. The current `methods` list is empty because no concrete signing method, physical approval integration, or signing implementation exists. |
| `get_accounts` | O | StackChan CoreS3 hardware smoke verifies deriving the Sui Ed25519 account (index 0, `m/44'/784'/0'/0'/0'`) from stored root material and returning it over an approved session while `provisioned`. Read-only: no mnemonic, seed, entropy, or private key in responses, logs, or UI. Derivation is also verified against Sui SDK address vectors on host (`test_sui_account_vectors.sh`). |
...
| `connect_device` | O | Gateway tool exists. StackChan CoreS3 hardware smoke verifies Firmware `connect` only after persistent root material, active policy, and `provisioned` exist. |
| `disconnect_device` | O | Hardware smoke verifies ending a live runtime session; Gateway tests cover stale local-session cleanup paths. |
| `get_capabilities` | O | Gateway tool and protocol parser exist. Hardware smoke verifies returning Firmware-authored Sui Ed25519 account identity capability over an active runtime session with `methods: []`; the parser rejects unsupported chains/methods/secret-like fields, and MCP never exposes the session id. |
| `get_accounts` | O | Gateway tool and protocol parser exist. Hardware smoke verifies returning public accounts over an active runtime session; the parser strictly re-validates the account shape, recomputes the Sui address/public-key relationship, rejects secret-like fields, and MCP never exposes the session id. |
...
| `call_method` | △ | Gateway tool and protocol parser exist for the common session-scoped method path. Hardware smoke verifies the Sui `sign_transaction` rejected policy-decision path through MCP and Firmware. It is not signing support, and MCP never exposes the session id. |
...
| Mnemonic UI flow v0 | △ | X | X | StackChan CoreS3 source adds DEV_PROFILE 12-word BIP-39 recovery phrase generation into RAM from an early-boot-seeded Agent-Q CSPRNG, device-only up-to-4-letter prefix display in 3 columns by 4 rows, persistent root material and active policy storage on confirmed backup, and wipe on cancel/confirm/reject/display expiry/timeout. Hardware smoke is still required. |
| `identify_device` | O | X | X | Uses temporary avatar speech bubble on StackChan CoreS3. |
| `connect` physical approval | O | X | X | StackChan CoreS3 hardware smoke verifies physical approval only after material-backed `provisioned`; before that the source returns `invalid_state`. |
| `disconnect` | O | X | X | StackChan CoreS3 hardware smoke verifies clearing a matching active Firmware session after `provisioned`. |
| `get_capabilities` | O | X | X | StackChan CoreS3 hardware smoke verifies reporting Sui Ed25519 account identity capability for account 0 over an approved session while `provisioned`; `methods` is empty until signing methods are implemented. |
| `get_accounts` | O | X | X | StackChan CoreS3 hardware smoke verifies deriving and returning the Sui Ed25519 account 0 over an approved session while `provisioned`; read-only, no private material emitted. |
...
| Mnemonic UI flow v0 | △ | StackChan CoreS3 source adds DEV_PROFILE BIP-39 recovery phrase generation from an early-boot-seeded Agent-Q CSPRNG, device-only up-to-4-letter prefix display, confirmed root material storage, and active default-reject policy initialization. It stores no mnemonic text, private key, or account data. Hardware smoke is still required. |
...
| Mnemonic generation/import | △ | DEV_PROFILE recovery phrase generation/display and persistent root material plus active policy storage source exists for StackChan CoreS3. Mnemonic import, USER_PROFILE secure root storage, and signing use remain unimplemented. |
