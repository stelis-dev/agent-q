...
routes explicitly in Client, host process, and Firmware. Sui is the only executable
...
Common Client, host process, and MCP request validation owns transport bounds and
...
| Runtime connection sessions | O | Held in host process memory only; session id is not exposed to MCP clients or the Admin Page. The local Admin Page and MCP tools share the same in-process `host processCore` session owner. |
