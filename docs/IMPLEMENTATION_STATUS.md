...
| Mnemonic UI flow v0 | △ | The local setup speech bubble generates a DEV_PROFILE BIP-39 phrase into RAM from an early-boot-seeded Agent-Q CSPRNG and displays only up-to-4-letter prefixes on device in a 3-column by 4-row grid with bottom Cancel/Confirm buttons. Three-letter BIP-39 words are displayed as the full word. Local Confirm stores root material, initializes the active default-reject policy, and wipes volatile scratch; local Cancel wipes volatile scratch. There is no USB protocol request for setup start, setup cancel, or backup confirmation. Hardware smoke is still required for the local setup UX. |
| Persistent material | △ | StackChan CoreS3 source stores the confirmed BIP-39 root entropy and active default-reject policy as ordinary DEV_PROFILE NVS blobs and reports `provisioned` only when both blobs and `prov_state=provisioned` agree. Existing DEV_PROFILE devices with `prov_state=provisioned` and valid root material but no policy record are migrated by installing the default-reject policy at boot. This is not USER_PROFILE encrypted storage. Hardware smoke is still required. |
| Local reset / material wipe | X | No normal product reset/recovery UX is implemented. Host-triggered reset/debug protocol paths are intentionally not implemented; material/state consistency errors fail closed until a local reset/recovery UX is specified and built. |
| `identify_device` | O | Implemented as temporary device UI for explicit user selection. |
| `connect` | O | Protocol and Gateway parser support exist. StackChan CoreS3 source gates Firmware session creation on material-backed `provisioned` plus physical approval; hardware smoke must be rerun after the local-only setup boundary change. |
| `disconnect` | O | Protocol and Gateway parser support exist. StackChan CoreS3 source clears only a matching active Firmware session after `provisioned`; hardware smoke must be rerun after the local-only setup boundary change. |
| `get_capabilities` | O | StackChan CoreS3 source reports Firmware-authored Sui Ed25519 account identity capability over an approved session while `provisioned`. The current `methods` list is empty because no concrete signing method, physical approval integration, or signing implementation exists. Hardware smoke must be rerun after the local-only setup boundary change. |
| `get_accounts` | O | StackChan CoreS3 source derives the Sui Ed25519 account (index 0, `m/44'/784'/0'/0'/0'`) from stored root material and returns it over an approved session while `provisioned`. Read-only: no mnemonic, seed, entropy, or private key in responses, logs, or UI. Derivation is verified against Sui SDK address vectors on host (`test_sui_account_vectors.sh`); hardware smoke must be rerun after the local-only setup boundary change. |
...
| `connect_device` | O | Gateway tool exists. Firmware `connect` is expected only after persistent root material, active policy, and `provisioned` exist; hardware smoke must be rerun after the local-only setup boundary change. |
| `disconnect_device` | O | Gateway tool exists and Gateway tests cover stale local-session cleanup paths. Hardware smoke must be rerun after the local-only setup boundary change. |
| `get_capabilities` | O | Gateway tool and protocol parser exist for Firmware-authored Sui Ed25519 account identity capability over an active runtime session with `methods: []`; the parser rejects unsupported chains/methods/secret-like fields, and MCP never exposes the session id. Hardware smoke must be rerun after the local-only setup boundary change. |
| `get_accounts` | O | Gateway tool and protocol parser exist for public accounts over an active runtime session; the parser strictly re-validates the account shape, recomputes the Sui address/public-key relationship, rejects secret-like fields, and MCP never exposes the session id. Hardware smoke must be rerun after the local-only setup boundary change. |
...
| `call_method` | △ | Gateway tool and protocol parser exist for the common session-scoped method path. The Sui `sign_transaction` rejected policy-decision path is implemented through MCP and Firmware source, but hardware smoke must be rerun after the local-only setup boundary change. It is not signing support, and MCP never exposes the session id. |
...
| Mnemonic UI flow v0 | △ | X | X | StackChan CoreS3 source adds DEV_PROFILE 12-word BIP-39 recovery phrase generation into RAM from an early-boot-seeded Agent-Q CSPRNG, device-only up-to-4-letter prefix display in 3 columns by 4 rows, persistent root material and active policy storage on local Confirm, and wipe on local Cancel/display expiry/failure. Hardware smoke is still required for the local setup UX. |
| `identify_device` | O | X | X | Uses temporary avatar speech bubble on StackChan CoreS3. |
| `connect` physical approval | O | X | X | StackChan CoreS3 source requires physical approval only after material-backed `provisioned`; before that the source returns `invalid_state`. Hardware smoke must be rerun after the local-only setup boundary change. |
| `disconnect` | O | X | X | StackChan CoreS3 source clears only a matching active Firmware session after `provisioned`. Hardware smoke must be rerun after the local-only setup boundary change. |
| `get_capabilities` | O | X | X | StackChan CoreS3 source reports Sui Ed25519 account identity capability for account 0 over an approved session while `provisioned`; `methods` is empty until signing methods are implemented. Hardware smoke must be rerun after the local-only setup boundary change. |
| `get_accounts` | O | X | X | StackChan CoreS3 source derives and returns the Sui Ed25519 account 0 over an approved session while `provisioned`; read-only, no private material emitted. Hardware smoke must be rerun after the local-only setup boundary change. |
...
| Provisioning flow | △ | X | X | DEV_PROFILE mnemonic UI and persistent root material plus active default-reject policy storage source exists for StackChan CoreS3. Public Sui account derivation is implemented via `get_accounts`; mnemonic import, signing use, policy update, and USER_PROFILE secure provisioning are not implemented. |
...
| Mnemonic UI flow v0 | △ | StackChan CoreS3 source adds DEV_PROFILE BIP-39 recovery phrase generation from an early-boot-seeded Agent-Q CSPRNG, device-only up-to-4-letter prefix display, local-confirmed root material storage, and active default-reject policy initialization. It stores no mnemonic text, private key, or account data. Hardware smoke is still required for the local setup UX. |
...
| Mnemonic generation/import | △ | DEV_PROFILE local recovery phrase generation/display and persistent root material plus active policy storage source exists for StackChan CoreS3. Mnemonic import, local reset/recovery UX, USER_PROFILE secure root storage, and signing use remain unimplemented. |
