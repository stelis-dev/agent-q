...
|---|---:|---:|---:|---:|---|
| StackChan CoreS3 | △ | X | X | X | Boot-time Sui Ed25519 self-test, read-only `get_accounts` account-0 derivation, and unified `sign_transaction` for the bounded restricted-transfer shape. Current-tree hardware smoke remains pending after the Sign API cutover. Broader Sui transaction shapes and other chains are not implemented. |
...
session-scoped `sign_transaction` protocol.
...
| Sui `sign_personal_message` | X | Not implemented. The Sui SDK signing substrate can define this intent, but Agent-Q does not yet have matching Firmware policy facts, device-local review rows, approval-history metadata, or Wallet Standard feature exposure for personal-message signing. Do not expose it until those contracts are implemented and tested together. |
