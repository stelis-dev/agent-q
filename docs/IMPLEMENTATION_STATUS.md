...
| Mnemonic UI flow v0 | △ | StackChan CoreS3 source adds DEV_PROFILE BIP-39 recovery phrase generation from an early-boot-seeded Agent-Q CSPRNG, device-only up-to-4-letter prefix display, local 6-digit PIN setup after backup confirmation, matching-PIN root material storage, active default-reject policy initialization, and salt + PIN verifier storage. It stores no mnemonic text, private key, or account data. The PIN verifier is not root encryption. Hardware smoke is still required for the local setup UX. |
...
| Mnemonic generation/import | △ | DEV_PROFILE local recovery phrase generation/display and persistent root material plus active policy plus local PIN verifier storage source exists for StackChan CoreS3. Mnemonic import, local reset/recovery UX, USER_PROFILE secure root storage, and signing use remain unimplemented. |
