...
| Provisioning status reporting | △ | X | X | StackChan CoreS3 firmware source reports `unprovisioned`, material-backed `provisioned`, or `error` for persistent material inconsistency. Manual smoke observed `provisioned` after local setup on StackChan CoreS3; failure and recovery states still need targeted hardware checks. |
...
| Local reset / material wipe | △ | X | X | StackChan CoreS3 source implements a device-local settings reset path for `provisioned` devices. Host-triggered reset/debug protocol paths are intentionally not implemented. Hardware smoke is still required. |
...
| Mnemonic generation/import | △ | DEV_PROFILE local recovery phrase generation/display and persistent root material plus active policy plus local PIN verifier storage source exists for StackChan CoreS3. Local reset/material wipe source exists separately for provisioned devices. Mnemonic import/recovery, USER_PROFILE secure root storage, and signing use remain unimplemented. |
