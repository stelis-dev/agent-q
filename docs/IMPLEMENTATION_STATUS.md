...
| `call_method` | △ | Runtime skeleton exists in Gateway and StackChan CoreS3 source. It is provisioned/session-scoped, keeps unknown methods rejected with `unsupported_method`, and recognizes Sui `sign_transaction` only for restricted-transfer policy-decision smoke. StackChan CoreS3 hardware smoke verifies that a valid supported transaction reaches the default-reject policy runtime and returns `policy_rejected`; host tests cover the Firmware `call_method` field/type validation boundary. No signature, physical approval, capability advertisement, or signing support is connected. |
...
| `connect_device` | O | Gateway tool exists. StackChan CoreS3 hardware smoke verifies Firmware `connect` only after persistent root material and `provisioned` exist. |
...
| `call_method` | △ | X | X | StackChan CoreS3 source enforces provisioned/session gates, keeps unknown methods rejected with `unsupported_method`, and recognizes Sui `sign_transaction` only to decode the restricted SUI transfer shape and consume the default-reject policy decision as a rejected method result. Hardware smoke verifies that rejected policy-decision path, and a host test covers the Firmware request field/type validation boundary. It does not ask for signing approval or sign. |
