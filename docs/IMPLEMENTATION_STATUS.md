...
| `disconnect` | O | Protocol and Gateway parser support exist. StackChan CoreS3 source clears only a matching active Firmware session; it does not require material readiness for session cleanup, but returns `busy` while a local setup/settings/PIN/reset flow is active so external teardown cannot perturb a device-local sensitive flow. Rerun hardware smoke after setup, session, or material-storage changes. |
...
| `disconnect` | O | X | X | StackChan CoreS3 source clears only a matching active Firmware session and is not gated on persistent material readiness. It may return `busy` while a local setup/settings/PIN/reset flow is active. Rerun hardware smoke after setup, session, or material-storage changes. |
...
| Local settings / material wipe | △ | X | X | StackChan CoreS3 source implements device-local settings actions for `provisioned` devices: connect PIN toggle, Change PIN, and Reset/material wipe. Host-triggered reset/debug/PIN-change protocol paths are intentionally not implemented. StackChan CoreS3 local reset was manually smoke-tested after commit `7c6e65c`; rerun hardware smoke after settings or reset UI/state changes. |
