...
| Signing APIs | X | No signing API exists. A future signing method must be exposed through common `call_method`, not chain-specific top-level MCP tools, and must update capability advertisement, Gateway parser/output schemas, Firmware approval/history handling, and provider API together before it is presented as available. |
