...
| Sui txBytes decoder | △ | Common firmware source includes a restricted host-tested SUI transfer facts parser for `TransactionData::V1 -> ProgrammableTransaction -> SplitCoins(GasCoin, [Input(amountPure)]) -> TransferObjects([NestedResult(0,0)], Input(recipientPure))`, matching the current Sui SDK transaction builder shape. Plain `Result(0)` references are tracked as unsupported parser fixtures, not compatibility input. StackChan CoreS3 connects that parser to Sui `sign_transaction` policy and user authorization gates. |
