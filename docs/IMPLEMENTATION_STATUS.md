...
| Policy storage/read | △ | X | X | StackChan CoreS3 stores the active policy as a canonical `agentq.policy.v0` binary record in two bounded NVS slots plus commit metadata and a pending-write marker. Backup confirmation plus matching local PIN repeat still installs only the DEV_PROFILE default-reject policy, but the storage/provider boundary can load canonical custom reject-policy records. It exposes a read-only `get_policy` summary, migrates legacy root-only missing policy to the default-reject record, preserves the previous committed policy only for interrupted writes identified by the pending marker, treats metadata flip as the commit point, classifies each write as applied, unchanged failure, or consistency error, tolerates stale pending markers that exactly match the selected committed policy, cleans legacy pending-owned modern residue before starting a new modern write, removes stale commit metadata before slot reuse, and treats corrupt/unreadable committed policy, invalid commit metadata without that marker, or pending targets that overlap active material without exactly matching it as a material-consistency error. Policy update authorization is not implemented. |
