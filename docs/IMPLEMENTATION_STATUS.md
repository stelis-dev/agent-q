...
| `call_method` | △ | X | X | StackChan CoreS3 source enforces provisioned/session gates, keeps unknown methods rejected with `unsupported_method`, and recognizes Sui `sign_transaction` only to decode the restricted SUI transfer shape and consume the committed active policy as a rejected method result. Corrupt/unreadable, missing, or unsupported current policy material fails closed as material inconsistency before normal session-scoped methods are available. A RAM-only method approval state owner exists for future signing approval wiring, but current `call_method` does not enter it. Host tests cover the Firmware request field/type validation boundary, policy-store provider behavior, and method approval state-owner transitions. Hardware smoke coverage exists for the stored-policy reject path. It does not ask for signing approval or sign. |
...
| Sui `sign_transaction` | △ | This method is recognized inside `call_method` only for policy-decision smoke. It validates `network` and base64 `txBytes`, decodes the restricted SUI transfer shape, consumes the committed active policy runtime decision, and returns a rejected `method_result`; corrupt/unreadable, missing, or unsupported current policy material fails closed as material consistency before normal session-scoped methods are available. It is not advertised in `get_capabilities`, does not sign, and does not trigger approval UI. A RAM-only method approval state owner exists for future signing approval wiring, but this method does not enter it. Hardware smoke coverage exists for this stored-policy reject path. Moving it to signing support requires capability advertisement, approved/signature result parsing, runtime wiring into the method approval state owner, policy `ask`/`sign` handling, required approval-history behavior, MCP output, and provider API changes in the same implementation boundary. |
