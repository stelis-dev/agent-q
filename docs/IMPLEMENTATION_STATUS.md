...
- `source-wired`: target runtime glue exists, but product exposure is still
  blocked. This status is not enough to advertise capabilities or document
  signing as available.
- `product-active`: public dispatcher, response writer, client/provider parser
  ...
  hardware smoke are all complete for the same contract.

Current `request_signature` status is `foundation` with public-inactive internal
partial runtime modules present. Do not use historical smoke, helper coverage,
or dormant source modules as evidence for `product-active`.
...
| `get_capabilities` | O | StackChan CoreS3 source reports Firmware-authored Sui Ed25519 account identity capability over an approved session while `provisioned`. Delegated public method capabilities are empty. Provider-facing signing availability is not advertised; `signatureRequests` is not part of the current public capability response. |
...
| `request_signature` | △ | StackChan CoreS3 source contains public-inactive internal partial runtime modules for a future provider-facing device-confirmed Sui `sign_transaction` request, but the current public USB dispatcher, Gateway core/client request builder and parser, provider `requestSignature` API, `signature_result` writer, and `signatureRequests` capability are inactive. The internal modules and host tests cover material/session/busy gating, `txBytes`-derived review facts, host-supplied `network` handling, local-PIN confirmation, required pre-signing history, signing-critical handoff, terminal metadata, and cleanup behavior. Those modules are not wired into the USB request-server runtime, so no public request can enter review, PIN, history, signing, or `signature_result` delivery. A target boundary guard checks that this public-inactive status does not accidentally regain USB runtime or public Gateway signing wiring. It is not an MCP signing tool, does not use `call_method`, and does not accept caller-controlled timing fields. Firmware-owned review/PIN input windows are modeled as a fixed internal 30-second window; stored-PIN cryptographic verification is not timed out by the local input window, the internal local-auth worker watchdog still fails closed as authentication unavailable, and wrong PIN results open a fresh 30-second input window only within the original fixed confirmation deadline unless the shared lockout is active. Current public product status is `foundation` with internal partial runtime modules present, not product-complete signing activation. Future activation must re-open dispatcher/client/provider/capability together and run current-tree target hardware smoke before claiming `product-active` status. |
...
| Signing APIs | X | Public provider-facing signing APIs are not active. Gateway core, the USB driver, the public protocol parser, and the provider package do not expose `requestSignature`, `request_signature`, `signature_result`, or `signatureRequests`. MCP has no signing tool and its get-capabilities runtime and exported output schema reject provider-facing signing metadata. Internal Firmware partial runtime modules for future device-confirmed signing remain tracked separately in the target `request_signature` row. |
...
| `get_capabilities` | O | Gateway tool and protocol parser exist for Firmware-authored Sui Ed25519 account identity capability over an active runtime session with `chains[].methods: []`; the parser rejects unsupported chains/methods, provider-facing `signatureRequests`, and secret-like fields. MCP never exposes the session id and does not expose a signing tool. |
...
| `get_approval_history` | △ | Gateway tool and protocol parser exist for read-only persistent Firmware approval history over an active runtime session. The parser accepts bounded method-decision records, policy-update terminal records, and current-schema `signature_request` confirmation/terminal records, preserves sequence and uptime as strings, rejects session ids and secret-like fields, and MCP never exposes the session id. `signature_request` history parsing is retained for the public-inactive internal partial runtime modules; public signing activation is not currently reachable. Opt-in policy-update hardware smoke coverage exists for newest policy-update terminal history over MCP. |
| `call_method` | △ | Gateway tool and protocol parser exist for the common session-scoped method path. Current public method results are rejected-only; approved signing through `call_method` is not accepted by the client/MCP schema. Device-confirmed signing is not currently exposed as an MCP tool or provider API. |
...
| `request_signature` | △ | X | X | StackChan CoreS3 source contains public-inactive internal partial runtime modules for a future provider-facing device-confirmed Sui `sign_transaction` signing request, but the public USB dispatcher, `signature_result` writer, and provider/client surfaces are inactive. The internal source modules and host tests cover material/session/busy gating, `txBytes`-derived review facts, local PIN, required confirmation history, signing-critical handoff, terminal metadata, and cleanup behavior. They are not wired into the USB request-server runtime. Future activation must re-open dispatcher, response writer, client/provider parser/API, capability, and current-tree hardware smoke together. |
...
| Per-request signing approval | △ | N/A | X | StackChan CoreS3 source contains public-inactive internal device-confirmed signing approval modules for future `request_signature` activation with local PIN and required pre-signing history. It is not currently reachable through a public provider/client/Firmware USB signing request. |
...
|---|---:|---:|---:|---:|---|
| StackChan CoreS3 | △ | X | X | X | Boot-time Sui Ed25519 self-test, read-only `get_accounts` account-0 derivation, and public-inactive internal request-signature partial runtime modules for the bounded restricted transfer shape. Public provider-facing Sui signing is not active. Broader Sui transaction shapes and other chains are not implemented. |
...
| Sui `sign_transaction` | △ | Through `call_method`, this method validates `network` and base64 `txBytes`, decodes the restricted SUI transfer shape, consumes the committed active policy runtime decision, rejects unsupported transactions, and returns rejected method results. Internal Firmware source has public-inactive request-signature partial runtime modules for the same bounded restricted transfer shape, but public provider-facing signing is not active. Corrupt/unreadable, missing, or invalid current policy material fails closed as material consistency before normal session-scoped methods are available. |
