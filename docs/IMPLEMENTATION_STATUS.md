...
| `connect` | O | Protocol and Gateway parser support exist. StackChan CoreS3 source gates Firmware session creation on material-backed `provisioned` plus physical approval; rerun hardware smoke after setup, session, or material-storage changes. |
| `disconnect` | O | Protocol and Gateway parser support exist. StackChan CoreS3 source clears only a matching active Firmware session after `provisioned`; rerun hardware smoke after setup, session, or material-storage changes. |
...
| Provisioning status reporting | △ | X | X | StackChan CoreS3 firmware source reports `unprovisioned` or material-backed `provisioned`. Manual smoke observed `provisioned` after local setup on StackChan CoreS3; failure and recovery states still need targeted hardware checks. |
| Mnemonic UI flow v0 | △ | X | X | StackChan CoreS3 source adds DEV_PROFILE 12-word BIP-39 recovery phrase generation into RAM from an early-boot-seeded Agent-Q CSPRNG, device-only up-to-4-letter prefix display in 3 columns by 4 rows, local 6-digit PIN setup after local Confirm, persistent root material/active policy/PIN verifier storage only after matching PIN repeat, and wipe on local Cancel/display expiry/PIN timeout/failure. StackChan CoreS3 local setup and PIN entry were manually smoke-tested after commit `2cb243b`; rerun hardware smoke after setup UI or state changes. |
| `identify_device` | O | X | X | Uses temporary avatar speech bubble on StackChan CoreS3. |
| `connect` physical approval | O | X | X | StackChan CoreS3 source requires physical approval only after material-backed `provisioned`; before that the source returns `invalid_state`. Rerun hardware smoke after setup, session, or material-storage changes. |
| `disconnect` | O | X | X | StackChan CoreS3 source clears only a matching active Firmware session after `provisioned`. Rerun hardware smoke after setup, session, or material-storage changes. |
...
| Mnemonic UI flow v0 | △ | StackChan CoreS3 source adds DEV_PROFILE BIP-39 recovery phrase generation from an early-boot-seeded Agent-Q CSPRNG, device-only up-to-4-letter prefix display, local 6-digit PIN setup after backup confirmation, matching-PIN root material storage, active default-reject policy initialization, and salt + PIN verifier storage. It stores no mnemonic text, private key, or account data. The PIN verifier is not root encryption. StackChan CoreS3 local setup and PIN entry were manually smoke-tested after commit `2cb243b`; rerun hardware smoke after setup UI or state changes. |
