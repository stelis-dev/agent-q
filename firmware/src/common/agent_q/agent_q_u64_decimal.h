...
constexpr size_t kAgentQU64DecimalMaxDigits = 20;
constexpr size_t kAgentQU64DecimalBufferBytes = kAgentQU64DecimalMaxDigits + 1;
constexpr const char* kAgentQU64DecimalMaxString = "18446744073709551615";
...
inline bool is_u64_decimal_string(const char* value)
{
    ...
    for (const char* cursor = value; *cursor != '\0'; ++cursor) {
        ...
        ++length;
        if (length > kAgentQU64DecimalMaxDigits) {
            return false;
        }
    }
    if (length == kAgentQU64DecimalMaxDigits &&
        strcmp(value, kAgentQU64DecimalMaxString) > 0) {
        return false;
    }
    return true;
}
...
inline bool format_u64_decimal(uint64_t value, char* output, size_t output_size)
{
    ...
    char scratch[kAgentQU64DecimalBufferBytes] = {};
