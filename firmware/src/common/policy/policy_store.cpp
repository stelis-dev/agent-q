...
constexpr size_t kDefaultPolicyRecordBytes = kCurrentPolicyDefaultCanonicalRecordBytes;

static CurrentPolicyCanonicalDocument* g_policy_document = nullptr;
static CurrentPolicyRuntimeView* g_policy_runtime_view = nullptr;
static uint8_t g_policy_record_buffer[kCurrentPolicyMaxCanonicalRecordBytes];
static uint8_t g_policy_encoded_buffer[kCurrentPolicyMaxCanonicalRecordBytes];
static uint8_t g_policy_scratch_buffer[kCurrentPolicyMaxCanonicalRecordBytes];
...
struct ActivePolicySelection {
    PolicyStoreStatus status;
    uint8_t slot;
    uint64_t sequence;
    size_t record_size;
};

PolicyStoreStatus select_active_policy(ActivePolicySelection* out, bool log_failures);
PolicyStoreStatus read_commit_record(uint8_t commit_index, PolicyCommit* out, bool log_failures);
PolicyStoreStatus read_pending_record(PolicyPendingWrite* out, bool log_failures);

bool ensure_policy_workspaces()
{
    if (g_policy_document == nullptr) {
        g_policy_document = static_cast<CurrentPolicyCanonicalDocument*>(
            malloc(sizeof(*g_policy_document)));
    }
    if (g_policy_runtime_view == nullptr) {
        g_policy_runtime_view = static_cast<CurrentPolicyRuntimeView*>(
...
void clear_policy_document_workspace()
{
    if (g_policy_document != nullptr) {
        memset(g_policy_document, 0, sizeof(*g_policy_document));
    }
}

void clear_policy_runtime_workspace()
{
    if (g_policy_runtime_view != nullptr) {
        memset(g_policy_runtime_view, 0, sizeof(*g_policy_runtime_view));
    }
}
...
{
    if (output == nullptr || output_size != kPolicyIdSize) {
        return false;
    }
...
{
    return encode_current_policy_default_record(output, output_capacity, output_size) ==
...
{
    if (record == nullptr || record_size == 0 ||
        record_size > kCurrentPolicyMaxCanonicalRecordBytes) {
        return false;
    }
    ...
    if (decode_current_policy_canonical_record(
            record,
            record_size,
            g_policy_document) != CurrentPolicyDocumentStatus::ok) {
        return false;
    }

    if (validate_current_policy_canonical_document(*g_policy_document) !=
        CurrentPolicyDocumentStatus::ok) {
        return false;
    }

    size_t encoded_size = 0;
    if (encode_current_policy_canonical_record(
            *g_policy_document,
            g_policy_encoded_buffer,
            sizeof(g_policy_encoded_buffer),
            &encoded_size) != CurrentPolicyDocumentStatus::ok ||
    ...
    bool log_failures)
{
    ...
    *out_size = 0;
    ...
    if (blob_size == 0 || blob_size > out_capacity) {
        nvs_close(nvs);
        return PolicyStoreStatus::invalid;
    }
...
{
    if (left.status != PolicyStoreStatus::active ||
        right.status != PolicyStoreStatus::active ||
...
bool selection_matches_written_policy(
    ...
    size_t record_size)
{
    return selection.status == PolicyStoreStatus::active &&
           selection.slot == slot &&
...
PolicyStoreWriteResult classify_policy_write_terminal_state(
    PolicyStoreStatus previous_status,
    ...
    bool may_cleanup_target)
{
    ...
    PolicyStoreStatus status = select_active_policy(&current, true);
    if (status == PolicyStoreStatus::active) {
        if (selection_matches_written_policy(current, slot, sequence, record, record_size)) {
            return PolicyStoreWriteResult::applied;
        }
        if (previous_status == PolicyStoreStatus::active &&
            selections_refer_to_same_policy(previous, current)) {
            return PolicyStoreWriteResult::unchanged_failure;
        }
        return PolicyStoreWriteResult::consistency_error;
    }
    if (previous_status != PolicyStoreStatus::active &&
    ...
    if (may_cleanup_target &&
        ...
        } else if (previous_status != PolicyStoreStatus::active &&
                   status == previous_status) {
            return PolicyStoreWriteResult::unchanged_failure;
        }
    }
...
PolicyStoreWriteResult classify_pending_write_failure_terminal_state(
    PolicyStoreStatus previous_status,
    const ActivePolicySelection& previous,
    uint8_t slot,
    uint8_t commit_index,
    ...
    size_t record_size)
{
    ...
    const PolicyStoreStatus pending_status = read_pending_record(&pending, true);
    if (pending_status == PolicyStoreStatus::storage_error ||
        pending_status == PolicyStoreStatus::invalid ||
        (pending.valid &&
...
PolicyStoreStatus read_policy_record_key(
    const char* key,
    uint8_t* out,
    size_t out_capacity,
    size_t* out_size,
    bool log_failures)
{
    const PolicyStoreStatus read_status =
        read_blob(key, out, out_capacity, out_size, log_failures);
    if (read_status != PolicyStoreStatus::active) {
        return read_status;
    }
...
PolicyStoreStatus read_policy_slot(
    uint8_t slot,
    uint8_t* out,
    size_t out_capacity,
    size_t* out_size,
...
PolicyStoreStatus read_commit_record(uint8_t commit_index, PolicyCommit* out, bool log_failures)
{
    ...
    *out = {};
    ...
    const PolicyStoreStatus status =
        read_blob(kPolicyCommitKeys[commit_index], commit, sizeof(commit), &commit_size, log_failures);
    if (status == PolicyStoreStatus::missing) {
        return PolicyStoreStatus::missing;
    }
    out->present = true;
    if (status == PolicyStoreStatus::storage_error) {
        return PolicyStoreStatus::storage_error;
    }
    if (status != PolicyStoreStatus::active || commit_size != sizeof(commit) ||
    ...
    uint8_t digest[32] = {};
    const PolicyStoreStatus slot_status =
        read_policy_slot(out->slot, g_policy_scratch_buffer, sizeof(g_policy_scratch_buffer), &record_size, log_failures);
    if (slot_status == PolicyStoreStatus::storage_error) {
        return PolicyStoreStatus::storage_error;
    }
    if (slot_status != PolicyStoreStatus::active ||
...
PolicyStoreStatus select_active_policy(ActivePolicySelection* out, bool log_failures)
{
    ...
    *out = {};
    out->status = PolicyStoreStatus::missing;

    PolicyPendingWrite pending = {};
    const PolicyStoreStatus pending_status = read_pending_record(&pending, log_failures);
    if (pending_status == PolicyStoreStatus::storage_error ||
        pending_status == PolicyStoreStatus::invalid) {
    ...
    for (uint8_t index = 0; index < 2; ++index) {
        const PolicyStoreStatus commit_status =
            read_commit_record(index, &commits[index], log_failures);
        if (commit_status == PolicyStoreStatus::storage_error) {
            out->status = commit_status;
            return out->status;
        }
        if (commit_status == PolicyStoreStatus::invalid &&
            commits[index].metadata_valid) {
            out->status = PolicyStoreStatus::invalid;
            return out->status;
        }
        if (commit_status == PolicyStoreStatus::invalid &&
    ...
    if (selected != nullptr) {
        ...
        if (slot_status == PolicyStoreStatus::storage_error ||
            slot_status == PolicyStoreStatus::invalid) {
        ...
        out->status = PolicyStoreStatus::active;
        return out->status;
    }
    ...
    for (uint8_t index = 0; index < 2; ++index) {
        ...
        if (slot_status == PolicyStoreStatus::storage_error) {
            out->status = PolicyStoreStatus::storage_error;
            return out->status;
        }
        slot_present[index] = slot_status != PolicyStoreStatus::missing;
        policy_material_present = policy_material_present || slot_present[index];
    }

    out->status = policy_material_present ? PolicyStoreStatus::invalid : PolicyStoreStatus::missing;
    return out->status;
}
...
bool store_default_policy()
{
    ...
    if (store_active_policy_record(record, record_size) != PolicyStoreWriteResult::applied) {
...
PolicyStoreWriteResult store_active_policy_record(const uint8_t* record, size_t record_size)
{
    if (!validate_policy_record(record, record_size)) {
        ...
        return PolicyStoreWriteResult::invalid_record;
    }

    ActivePolicySelection current = {};
    const PolicyStoreStatus current_status = select_active_policy(&current, true);
    if (current_status == PolicyStoreStatus::storage_error ||
        current_status == PolicyStoreStatus::invalid) {
        return PolicyStoreWriteResult::consistency_error;
    }

    const uint8_t next_slot =
        current_status == PolicyStoreStatus::active && current.slot == 0 ? 1 : 0;
    const uint64_t next_sequence =
        current_status == PolicyStoreStatus::active ? current.sequence + 1 : 1;
    if (next_sequence == 0) {
        return PolicyStoreWriteResult::consistency_error;
    }
    ...
    if (select_active_policy(&after, true) != PolicyStoreStatus::active ||
...
bool read_active_policy_summary(StoredPolicySummary* out)
{
    ...
    ActivePolicySelection selection = {};
    if (select_active_policy(&selection, true) != PolicyStoreStatus::active ||
        decode_current_policy_canonical_record(
...
bool read_active_policy_document(StoredPolicyDocument* out)
{
    ...
    if (select_active_policy(&selection, true) != PolicyStoreStatus::active ||
        decode_current_policy_canonical_record(
            ...
            g_policy_document) != CurrentPolicyDocumentStatus::ok ||
        !policy_id_for_record(g_policy_record_buffer, selection.record_size, out->policy_id, sizeof(out->policy_id)) ||
        !current_policy_canonical_to_runtime_view(*g_policy_document, g_policy_runtime_view)) {
        return false;
    }
