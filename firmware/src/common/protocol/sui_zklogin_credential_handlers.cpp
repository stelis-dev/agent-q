...
const char* prepare_result_error_code(SuiZkLoginCredentialPrepareResult result)
{
    switch (result) {
        case SuiZkLoginCredentialPrepareResult::invalid_state:
            return "invalid_state";
        case SuiZkLoginCredentialPrepareResult::invalid_session:
            return "invalid_session";
        case SuiZkLoginCredentialPrepareResult::internal_output_error:
        case SuiZkLoginCredentialPrepareResult::ok:
        default:
...
bool preparation_public_key_base64(
    const SuiZkLoginCredentialPreparation& preparation,
...
void write_proposal_begin_failure(
    ...
    SuiZkLoginCredentialProposalBeginResult result)
{
    if (ops.on_proposal_begin_failure != nullptr) {
        ops.on_proposal_begin_failure();
    }
    if (result == SuiZkLoginCredentialProposalBeginResult::invalid_state) {
        writer.write_error(id, "invalid_state");
        return;
    }
    if (result == SuiZkLoginCredentialProposalBeginResult::internal_output_error) {
    ...
    const SuiZkLoginCredentialHandlerOps& ops)
{
    ...
    SuiZkLoginCredentialPreparation preparation = {};
    const SuiZkLoginCredentialPrepareResult prepare_result =
        ops.prepare_credential(session_id, &preparation);
    if (prepare_result != SuiZkLoginCredentialPrepareResult::ok) {
    ...
    const SuiZkLoginCredentialHandlerOps& ops)
{
    ...
    const SuiZkLoginCredentialProposalBeginResult begin_result =
        ops.begin_proposal(
            request["payload"],
            id,
            session_id,
            now,
            request_window);
    if (begin_result != SuiZkLoginCredentialProposalBeginResult::ok) {
