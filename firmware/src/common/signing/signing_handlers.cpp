    ...
    bool wrote_response)
{
    switch (status) {
        case PolicySigningExecutionStatus::request_error:
            if (!wrote_response) {
                return "Request failed; response failed";
            }
            ...
            return "Unsupported transaction";
        case PolicySigningExecutionStatus::policy_rejected:
            return wrote_response ? "Policy rejected" : "Rejected; response failed";
        case PolicySigningExecutionStatus::signing_failed:
            return wrote_response ? "Signing failed" : "Failure; response failed";
        case PolicySigningExecutionStatus::signed_success:
            return wrote_response ? "Policy signed" : "Signed; response failed";
        default:
            return nullptr;
    }
}

SigningNoticeKind policy_execution_notice_kind(
    PolicySigningExecutionStatus status,
    bool wrote_response)
{
    if (!wrote_response ||
    ...
    const SigningHandlerOps& ops)
{
    ...
    if (policy_result.status == SignTransactionPolicyRuntimeStatus::policy_authorized) {
        show_policy_notice(ops, "Policy signing", SigningNoticeKind::info);
    }
...
void handle_protocol_sign_transaction_request(
    const char* id,
...
void handle_protocol_sign_personal_message_request(
    const char* id,
    JsonDocument& request,
    const ResponseWriter& writer,
    const SigningHandlerOps& ops)
