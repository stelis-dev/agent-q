    ...
    const JsonDocument& response)
{
    ...
    if (!signing_outcome_response_fits(response)) {
        return false;
    }
    ...
    const SigningOutcomeWriterOps& ops)
{
    ...
    return json_response_write(response, ops.response_write_ops);
}
...
{
    JsonDocument response;
    if (!signing_outcome_prepare_signed_response(
            response,
            id,
            authorization,
            signing_route,
    ...
    const UserSigningOutput& signing_output)
{
    ...
    return signing_outcome_response_fits(response);
}
