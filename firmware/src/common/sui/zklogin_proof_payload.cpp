...
bool parse_proof_points(JsonVariantConst value, SuiZkLoginProofPoints* output)
{
    ...
    if (!json_object_fields_supported(object, kKeys, 3) ||
...
bool parse_inputs(JsonVariantConst value, SuiZkLoginSignatureInputs* output)
{
    ...
    if (!json_object_fields_supported(object, kKeys, 4) ||
    ...
    if (!json_object_fields_supported(details, kDetailKeys, 2) ||
        !copy_nonempty_limited(
...
{
    if (record == nullptr || !payload.is<JsonObjectConst>()) {
        return SuiZkLoginProofPayloadParseResult::invalid_argument;
    }
    ...
    if (!json_object_fields_supported(object, kKeys, 7) ||
        ...
        memset(record, 0, sizeof(*record));
        return SuiZkLoginProofPayloadParseResult::invalid_proof;
    }
    if (!compute_proof_hash(record)) {
        memset(record, 0, sizeof(*record));
        return SuiZkLoginProofPayloadParseResult::encode_error;
    }
    if (!validate_sui_zklogin_proof_record(record)) {
        memset(record, 0, sizeof(*record));
        return SuiZkLoginProofPayloadParseResult::invalid_proof;
    }
    return SuiZkLoginProofPayloadParseResult::ok;
}
