...
`Settings > Sui` display/clear, and final zkLogin signature-envelope
...
| Local settings menu | Temporary settings menu with a scrollable label/control list. Current implemented actions are human approval input mode toggle, signing authorization mode toggle, `Settings > Sui` active identity/proof display with local proof clear, policy reset, Change PIN, and Reset. Each sensitive write or clear action opens local PIN verification directly. | `idle` while the menu is idle; `busy` after entering a sensitive subflow |
