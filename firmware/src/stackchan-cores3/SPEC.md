...
wear. Required method-signing terminal history records and required
policy-update terminal history records are not consumed from that
method-decision spam budget; those flows have their own active session, request
validation, and device-local approval gates before they can emit them. If a
current budgeted method decision requires a history record and the write fails
or the write budget is exhausted, `call_method` returns top-level
