...
or user confirmation internally for transaction signing. Invalid, unbindable,
unsupported-version/kind, `TransactionKind`-only, malformed, trailing, or
oversized Sui transaction bytes fail closed. Valid account-bound transactions
whose offline facts review coverage is incomplete may enter explicit user-mode
blind signing. Those Sign API paths have
...
policy for Sui `sign_transaction` policy evaluation only after the parsed
transaction shape has complete policy coverage. Valid policy-incomplete Sui
transactions return `policy_rejected` before signing.
...
| Policy evaluation | △ | Links the common host-tested policy evaluator, stored-policy provider boundary, and Sui method adapter over Firmware-derived `TransactionData` facts. The current Sui policy fact descriptor covers a bounded offline `TransactionData::V1 -> ProgrammableTransaction` fact set: sender, gas owner, gas price, gas budget, expiration kind, command count, command kinds, and MoveCall package/module/function/type-argument facts for each covered MoveCall command. That descriptor is not sufficient signing coverage: Sui `sign_transaction` policy mode returns `policy_rejected` for valid policy-incomplete transactions and does not sign until Firmware has complete policy coverage for actual transaction values and a committed active policy can safely return `sign`. Current accepted active policy records may contain `reject` rules only for Sui `sign_transaction`; policy `sign` rules are rejected before storage. |
...
| Sui `sign_transaction` | △ | Through `sign_transaction`, this method validates request-context `network` and either canonical base64 inline `txBytes` or a same-session staged `payloadRef`, derives offline-provable facts with the current Sui `TransactionData::V1 -> ProgrammableTransaction` facts extractor, and then uses the Firmware-local signing authorization mode only if the parsed shape has mode-specific authorization coverage. Policy mode evaluates the committed active policy only for policy-covered shapes and returns `policy_rejected` for valid policy-incomplete transactions; user mode shows covered offline facts when offline facts review coverage is complete, or a blind-signing warning when Firmware can validate and bind the transaction but offline facts review coverage is incomplete. Product-active status is tracked in `docs/IMPLEMENTATION_STATUS.md`. Corrupt/unreadable or missing policy fails closed before policy-mode signing is available. |
