...
have `source-wired-not-product-active` status in the current tree. Current-tree
direct USB/Firmware hardware smoke has passed for the implemented Sign API
outcomes. LVGL clear-signing visual evidence remains pending, so
...
`source-wired-not-product-active` status. Current-tree direct USB/Firmware
hardware smoke has passed for implemented Sign API outcomes, and LVGL visual
evidence remains pending before product-active status. Firmware-local signing
...
- rerun the tracked client-owned signing smoke harness when Sign API source,
  protocol, or target runtime changes. Current-tree direct USB/Firmware smoke
  has covered the `source-wired-not-product-active` Sign API paths:
