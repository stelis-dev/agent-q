...
`source-wired-not-product-active` status. Current-tree
positive/reject/timeout/session-loss smoke coverage for the current Sign API
wire names remains pending. Firmware-local signing mode selects policy
authorization or user confirmation for transaction signing.
...
- extend the tracked client-owned signing smoke harness where needed, then run
  it for the `source-wired-not-product-active` Sign API paths:
