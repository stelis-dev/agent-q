...
transaction matches the current automatic policy-signing contract. Valid
policy-incomplete Sui transactions return `policy_rejected` before signing.
...
| Policy evaluation | △ | Links the common host-tested policy evaluator, stored-policy provider boundary, and Sui method adapter over Firmware-derived `TransactionData` facts. The current Sui policy fact descriptor covers a bounded offline `TransactionData::V1 -> ProgrammableTransaction` fact set: sender, gas owner, gas price, gas budget, expiration kind, command count, command kinds, MoveCall package/module/function/type-argument facts for each covered MoveCall command, and token-flow fields for the current automatic policy-signing contract. Sui `sign_transaction` policy mode can sign only when Firmware proves the transaction is the current GasCoin-derived proven-SUI split-result transfer shape and a committed active policy has one matching bounded `sign` rule. Valid policy-incomplete transactions return `policy_rejected`. |
...
this target. Full host process Admin policy editing beyond the current Sui
transfer policy example is not implemented.
