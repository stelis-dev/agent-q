...
AgentQLocalAuthWorkerResult g_terminal_failure_result = {};
...
void clear_request(AgentQLocalAuthWorkerRequest* request)
{
    if (request == nullptr) {
        return;
    }
    request->job_id = 0;
    request->owner = AgentQLocalAuthWorkerOwner::provisioning_setup;
    request->operation = AgentQLocalAuthWorkerOperation::verify_pin;
}

void store_terminal_failure_result(const AgentQLocalAuthWorkerRequest& request)
{
    ...
    g_terminal_failure_result.status = AgentQLocalAuthWorkerStatus::worker_unavailable;
    ...
    uint32_t* job_id)
{
    ...
    AgentQLocalAuthWorkerRequest request = {};
...
void local_auth_worker_task(void*)
{
    while (true) {
        ...
        AgentQLocalAuthWorkerResult result = {};
        result.job_id = request.job_id;
        result.owner = request.owner;
        result.operation = request.operation;
        result.status = AgentQLocalAuthWorkerStatus::worker_unavailable;

        if (request.operation == AgentQLocalAuthWorkerOperation::verify_pin) {
            bool verified = false;
            if (verify_local_pin(pin, &verified)) {
                result.status = AgentQLocalAuthWorkerStatus::ok;
                result.verified = verified;
            } else {
                result.status = AgentQLocalAuthWorkerStatus::auth_unavailable;
            }
        } else if (request.operation == AgentQLocalAuthWorkerOperation::prepare_verifier_record) {
...
bool local_auth_worker_init()
{
    if (g_request_queue == nullptr) {
        g_request_queue = xQueueCreate(1, sizeof(AgentQLocalAuthWorkerRequest));
        if (g_request_queue == nullptr) {
    ...
    if (g_result_queue == nullptr) {
        g_result_queue = xQueueCreate(1, sizeof(AgentQLocalAuthWorkerResult));
        if (g_result_queue == nullptr) {
...
bool local_auth_worker_submit_verify(
    ...
    uint32_t* job_id)
{
    return submit_request(owner, AgentQLocalAuthWorkerOperation::verify_pin, pin, job_id);
}
...
bool local_auth_worker_submit_prepare_verifier(
    ...
    uint32_t* job_id)
{
    return submit_request(owner, AgentQLocalAuthWorkerOperation::prepare_verifier_record, pin, job_id);
}
