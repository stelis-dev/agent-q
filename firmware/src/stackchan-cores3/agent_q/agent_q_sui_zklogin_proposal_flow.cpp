...
constexpr size_t kBase64ZkLoginPublicKeyMaxChars =
    ((kAgentQSuiZkLoginPublicKeyMaxBytes + 2) / 3) * 4;
constexpr size_t kDecodedClaimMaxBytes = 384;

struct AgentQSuiZkLoginProposalState {
    bool active = false;
    AgentQSuiZkLoginProposalStage stage = AgentQSuiZkLoginProposalStage::idle;
    ...
    AgentQSuiZkLoginProofRecord record = {};

    void clear()
    {
        memset(this, 0, sizeof(*this));
        stage = AgentQSuiZkLoginProposalStage::idle;
        request_window = kAgentQTimeoutWindowNone;
    }
};

AgentQSuiZkLoginProposalState g_state;
    ...
    size_t output_size)
{
    ...
    if (input_length < 2 || input_length >= kAgentQSuiZkLoginIssBase64BufferSize) {
        return false;
    }

    uint8_t bits[kAgentQSuiZkLoginIssBase64BufferSize * 6] = {};
    size_t bit_count = 0;
...
bool extract_issuer(
    const AgentQSuiZkLoginIssBase64Details& details,
    char* issuer,
    size_t issuer_size)
{
    ...
    char extracted[kAgentQSuiZkLoginIssuerBufferSize] = {};
...
bool parse_proof_point_array(
    ...
    char output[][kAgentQSuiZkLoginProofPointBufferSize])
{
...
bool parse_proof_points(
    JsonVariantConst value,
    AgentQSuiZkLoginProofPoints* output)
{
    ...
    if (!object_has_only_keys(object, kKeys, 3) ||
        !parse_proof_point_array(
            object["a"],
            kAgentQSuiZkLoginProofPointACount,
            output->a) ||
        !parse_proof_point_array(
            object["c"],
            kAgentQSuiZkLoginProofPointCCount,
            output->c) ||
        !object["b"].is<JsonArrayConst>()) {
        return false;
    }
    JsonArrayConst b = object["b"].as<JsonArrayConst>();
    if (b.size() != kAgentQSuiZkLoginProofPointBOuterCount) {
        return false;
    }
    for (size_t row = 0; row < kAgentQSuiZkLoginProofPointBOuterCount; ++row) {
        if (!parse_proof_point_array(
                b[row],
                kAgentQSuiZkLoginProofPointBInnerCount,
                output->b[row])) {
            return false;
        }
    }
...
bool parse_inputs(
    JsonVariantConst value,
    AgentQSuiZkLoginSignatureInputs* output)
{
    if (output == nullptr || !value.is<JsonObjectConst>()) {
        return false;
    }
...
bool append_hash_proof_points(
    ...
    const AgentQSuiZkLoginProofPoints& points)
{
    for (size_t index = 0; index < kAgentQSuiZkLoginProofPointACount; ++index) {
        if (!append_hash_string(context, points.a[index])) {
            return false;
        }
    }
    for (size_t row = 0; row < kAgentQSuiZkLoginProofPointBOuterCount; ++row) {
        for (size_t column = 0; column < kAgentQSuiZkLoginProofPointBInnerCount; ++column) {
            if (!append_hash_string(context, points.b[row][column])) {
                return false;
            }
        }
    }
    for (size_t index = 0; index < kAgentQSuiZkLoginProofPointCCount; ++index) {
...
bool compute_proof_hash(AgentQSuiZkLoginProofRecord* record)
{
    if (record == nullptr) {
        return false;
    }
    ...
    const bool ok =
        ...
        append_hash_string(&context, "agentq.sui.zklogin.proof.v0") &&
...
bool parse_public_key(const char* base64, AgentQSuiZkLoginProofRecord* record)
{
    ...
    if (!validate_canonical_base64_syntax(
            ...
            &decoded_size) ||
        decoded_size < kAgentQSuiZkLoginPublicKeyMinBytes ||
        decoded_size > kAgentQSuiZkLoginPublicKeyMaxBytes ||
        base64_to_bytes(
            base64,
            strlen(base64),
            record->public_key,
...
AgentQSuiZkLoginProposalBeginResult parse_params(
    JsonVariantConst params,
    AgentQSuiZkLoginProofRecord* record)
{
    if (record == nullptr || !params.is<JsonObjectConst>()) {
        return AgentQSuiZkLoginProposalBeginResult::invalid_argument;
    }
    ...
    if (!object_has_only_keys(object, kKeys, 7) ||
        ...
        memset(record, 0, sizeof(*record));
        return AgentQSuiZkLoginProposalBeginResult::invalid_proof;
    }
    if (!compute_proof_hash(record)) {
        memset(record, 0, sizeof(*record));
        return AgentQSuiZkLoginProposalBeginResult::encode_error;
    }
    if (!validate_sui_zklogin_proof_record(record)) {
        memset(record, 0, sizeof(*record));
        return AgentQSuiZkLoginProposalBeginResult::invalid_proof;
    }
    return AgentQSuiZkLoginProposalBeginResult::ok;
}
...
{
    return AgentQSuiZkLoginProposalSnapshot{
        g_state.active,
        g_state.stage,
        g_state.request_id,
        g_state.session_id,
    ...
    AgentQTimeoutWindow request_window)
{
    ...
    const AgentQSuiZkLoginProposalBeginResult parse_result =
        parse_params(params, &g_state.record);
    if (parse_result != AgentQSuiZkLoginProposalBeginResult::ok) {
        g_state.clear();
        return parse_result;
    }
    g_state.request_window = request_window;
    g_state.stage = AgentQSuiZkLoginProposalStage::reviewing;
    g_state.active = true;
    return AgentQSuiZkLoginProposalBeginResult::ok;
}
...
{
    if (!g_state.active) {
        return AgentQSuiZkLoginProposalTransitionResult::inactive;
    }
    if (g_state.stage != AgentQSuiZkLoginProposalStage::reviewing) {
        return AgentQSuiZkLoginProposalTransitionResult::wrong_stage;
    }
    if (timeout_window_reached(g_state.request_window, now)) {
        return AgentQSuiZkLoginProposalTransitionResult::timed_out;
    }
    g_state.stage = AgentQSuiZkLoginProposalStage::pin_entry;
    return AgentQSuiZkLoginProposalTransitionResult::ok;
}
    ...
    TickType_t now)
{
    ...
    if (g_state.stage != AgentQSuiZkLoginProposalStage::pin_entry) {
        return AgentQSuiZkLoginProposalTransitionResult::wrong_stage;
    }
    if (timeout_window_reached(g_state.request_window, now)) {
        return AgentQSuiZkLoginProposalTransitionResult::timed_out;
...
AgentQSuiZkLoginProposalTransitionResult sui_zklogin_proposal_flow_mark_pin_verifying()
{
    ...
    g_state.stage = AgentQSuiZkLoginProposalStage::pin_verifying;
    return AgentQSuiZkLoginProposalTransitionResult::ok;
}
...
AgentQSuiZkLoginProposalTransitionResult sui_zklogin_proposal_flow_return_to_pin_entry()
{
    ...
    if (g_state.stage != AgentQSuiZkLoginProposalStage::pin_verifying) {
        return AgentQSuiZkLoginProposalTransitionResult::wrong_stage;
    }
    g_state.stage = AgentQSuiZkLoginProposalStage::pin_entry;
    return AgentQSuiZkLoginProposalTransitionResult::ok;
...
{
    if (!g_state.active || g_state.stage != AgentQSuiZkLoginProposalStage::pin_verifying) {
        return AgentQSuiZkLoginProposalTerminalResult::invalid_state;
    }
    g_state.stage = AgentQSuiZkLoginProposalStage::committing;
    switch (store_sui_zklogin_proof_record(&g_state.record)) {
        case AgentQSuiZkLoginProofRecordWriteResult::stored:
            return AgentQSuiZkLoginProposalTerminalResult::activated;
        case AgentQSuiZkLoginProofRecordWriteResult::storage_error:
            return AgentQSuiZkLoginProposalTerminalResult::storage_error;
        case AgentQSuiZkLoginProofRecordWriteResult::consistency_error:
            return AgentQSuiZkLoginProposalTerminalResult::consistency_error;
        case AgentQSuiZkLoginProofRecordWriteResult::invalid_record:
        default:
            return AgentQSuiZkLoginProposalTerminalResult::invalid_proof;
    }
}
...
{
    switch (result) {
        case AgentQSuiZkLoginProposalBeginResult::encode_error:
            return "encode_error";
        case AgentQSuiZkLoginProposalBeginResult::invalid_argument:
            return "invalid_argument";
        case AgentQSuiZkLoginProposalBeginResult::invalid_proof:
        case AgentQSuiZkLoginProposalBeginResult::ok:
        default:
    ...
    AgentQSuiZkLoginProposalTerminalResult result)
{
    switch (result) {
        case AgentQSuiZkLoginProposalTerminalResult::activated:
            return "activated";
        case AgentQSuiZkLoginProposalTerminalResult::rejected:
            return "rejected";
        case AgentQSuiZkLoginProposalTerminalResult::timed_out:
            return "timed_out";
        case AgentQSuiZkLoginProposalTerminalResult::invalid_proof:
            return "invalid_proof";
        case AgentQSuiZkLoginProposalTerminalResult::ui_error:
            return "ui_error";
        case AgentQSuiZkLoginProposalTerminalResult::storage_error:
            return "storage_error";
        case AgentQSuiZkLoginProposalTerminalResult::consistency_error:
            return "consistency_error";
        case AgentQSuiZkLoginProposalTerminalResult::invalid_state:
        default:
            return "";
    }
}
