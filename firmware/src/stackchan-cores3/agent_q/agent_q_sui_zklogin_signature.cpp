...
bool append_string_vector(
    BcsWriter* writer,
    const char values[][kAgentQSuiZkLoginProofPointBufferSize],
    size_t count)
{
    ...
    for (size_t index = 0; index < count; ++index) {
        if (!append_string(writer, values[index], kAgentQSuiZkLoginProofPointBufferSize - 1)) {
            return false;
        }
    }
    return true;
...
bool append_proof_points(BcsWriter* writer, const AgentQSuiZkLoginProofPoints& proof_points)
{
    if (!append_string_vector(
            writer,
            proof_points.a,
            kAgentQSuiZkLoginProofPointACount) ||
        !append_uleb128(writer, kAgentQSuiZkLoginProofPointBOuterCount)) {
        return false;
    }
...
bool append_signature_inputs(
    BcsWriter* writer,
    const AgentQSuiZkLoginSignatureInputs& inputs)
{
    ...
    size_t* output_size_written)
{
    ...
    if (output_size < kSuiSignatureEnvelopeMaxBytes) {
        return AgentQSuiZkLoginSignatureBuildResult::output_too_small;
    }
    if (user_signature == nullptr ||
        user_signature_size != kSuiEd25519SignatureBytes ||
        user_signature[0] != kAgentQSuiSignatureSchemeFlagEd25519) {
        return AgentQSuiZkLoginSignatureBuildResult::invalid_input;
    }
    ...
    if (!append_signature_inputs(&writer, inputs) ||
        ...
        clear_output(output, output_size, output_size_written);
        return AgentQSuiZkLoginSignatureBuildResult::bcs_too_large;
    }
    if (writer.cursor <= 1 ||
...
const char* sui_zklogin_signature_build_result_name(
    AgentQSuiZkLoginSignatureBuildResult result)
{
    switch (result) {
        case AgentQSuiZkLoginSignatureBuildResult::ok:
            return "ok";
        case AgentQSuiZkLoginSignatureBuildResult::invalid_input:
            return "invalid_input";
        case AgentQSuiZkLoginSignatureBuildResult::output_too_small:
            return "output_too_small";
        case AgentQSuiZkLoginSignatureBuildResult::bcs_too_large:
