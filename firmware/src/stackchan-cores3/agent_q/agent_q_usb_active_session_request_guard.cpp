...
bool parse_session_id_or_write_error(
    ...
    AgentQUsbSessionIdMode mode,
    const char** session_id)
{
    const bool parsed =
        mode == AgentQUsbSessionIdMode::required
            ? agent_q_json_value_c_string(request["sessionId"], session_id)
            : agent_q_json_optional_c_string(request["sessionId"], "", session_id);
    if (parsed) {
        return true;
    }
