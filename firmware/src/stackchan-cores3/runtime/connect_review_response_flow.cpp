...
bool local_pin_flow_active(const ConnectReviewResponseFlowOps& ops)
{
    return ops.local_pin_flow_active != nullptr && ops.local_pin_flow_active();
}

void reset_connect_choices(const ConnectReviewResponseFlowOps& ops)
{
...
void send_connect_terminal_response_if_needed(
    const ConnectReviewResponseFlowOps& ops)
{
    const ConnectApprovalSnapshot approval =
        ops.snapshot != nullptr ? ops.snapshot() : ConnectApprovalSnapshot{};
