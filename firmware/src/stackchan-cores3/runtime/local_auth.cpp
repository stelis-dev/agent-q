...
static_assert(
    sizeof(StoredLocalAuthRecord) == kLocalAuthPreparedRecordBytes,
    "Prepared local auth record shape must match stored record");
...
LocalAuthStatus read_local_auth_record(StoredLocalAuthRecord* out, bool log_failures)
{
    if (out == nullptr) {
        return LocalAuthStatus::storage_error;
    }
    ...
    if (result != ESP_OK) {
        if (result == ESP_ERR_NVS_NOT_FOUND) {
            return LocalAuthStatus::missing;
        }
        if (log_failures) {
    ...
    if (record_size != sizeof(*out)) {
        ...
        return LocalAuthStatus::invalid;
    }
...
bool active_record_matches(const StoredLocalAuthRecord& expected)
{
    ...
    const bool matches =
        read_local_auth_record(&current, false) == LocalAuthStatus::active &&
...
bool stored_pin_verifies(const char* pin, bool* verified, bool log_failures)
{
    ...
    StoredLocalAuthRecord record = {};
    if (read_local_auth_record(&record, log_failures) != LocalAuthStatus::active) {
        return false;
    }
...
bool prepare_local_pin_verifier_record(const char* pin, LocalAuthPreparedRecord* out)
{
    ...
    wipe_local_pin_verifier_record(out);
    ...
    memcpy(out->bytes, &record, sizeof(record));
    wipe_sensitive_buffer(&record, sizeof(record));
    return true;
}
...
bool store_prepared_local_pin_verifier(const LocalAuthPreparedRecord* prepared)
{
    ...
    const bool had_previous_record =
        read_local_auth_record(&previous_record, false) == LocalAuthStatus::active;

    StoredLocalAuthRecord record = {};
    memcpy(&record, prepared->bytes, sizeof(record));
    if (!validate_local_auth_record(record)) {
        ESP_LOGW(kTag, "Refusing invalid prepared local PIN verifier record");
    ...
    if (result != ESP_OK) {
        ...
        memcpy(&record, prepared->bytes, sizeof(record));
        if (active_record_matches(record)) {
            wipe_sensitive_buffer(&record, sizeof(record));
            wipe_sensitive_buffer(&previous_record, sizeof(previous_record));
        ...
        if (!had_previous_record && local_auth_status() == LocalAuthStatus::missing) {
            wipe_sensitive_buffer(&previous_record, sizeof(previous_record));
            return false;
        }
...
{
    LocalAuthPreparedRecord prepared = {};
    if (!prepare_local_pin_verifier_record(pin, &prepared)) {
        return false;
    }
    const bool stored = store_prepared_local_pin_verifier(&prepared);
    wipe_local_pin_verifier_record(&prepared);
    return stored;
}
...
{
    StoredLocalAuthRecord record = {};
    const LocalAuthStatus status = read_local_auth_record(&record, false);
    wipe_sensitive_buffer(&record, sizeof(record));
    return status;
}
