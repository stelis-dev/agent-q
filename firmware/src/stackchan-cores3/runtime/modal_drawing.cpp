...
ModalDrawingCallbacks g_callbacks;
...
static bool make_user_signing_review_row(
    lv_obj_t* parent,
    const UserSigningReviewRow& row,
    int y)
{
    lv_obj_t* label = lv_label_create(parent);
    if (label == nullptr) {
    ...
    const bool warning = row.kind == UserSigningReviewRowKind::warning;
    const bool section = row.kind == UserSigningReviewRowKind::section;
    ...
    const bool wrapped_value =
        row.kind == UserSigningReviewRowKind::wrapped_value ||
        row.kind == UserSigningReviewRowKind::section;
    ...
    TimeoutWindow timeout_window)
{
    ...
    lv_obj_t* panel = drawing_surface_create_panel_locked(UiPanelKind::connect_review);
    if (panel == nullptr) {
        return false;
    }
    ...
    lv_label_set_text(
        mode_value,
        input_mode == HumanApprovalInputMode::pin
            ? "PIN"
            : "Device confirm");
    ...
    if (!make_setup_button(
        ...
        !make_setup_button(
            panel,
            input_mode == HumanApprovalInputMode::pin ? "Continue" : "Confirm",
...
bool modal_draw_setup_choice_panel()
{
    ...
    const signing::ProvisioningFlowSnapshot flow =
        signing::provisioning_flow_snapshot();
    avatar_overlay_clear();
    ...
    lv_obj_t* panel = drawing_surface_create_panel_locked(UiPanelKind::setup_choice);
    if (panel == nullptr) {
        return false;
    }
    lv_obj_remove_flag(panel, LV_OBJ_FLAG_SCROLLABLE);
...
static bool make_import_word_cell(lv_obj_t* parent, uint8_t slot)
{
    ...
    const size_t global_slot =
        signing::provisioning_flow_import_global_word_slot_for(flow.import_page, slot);
    if (global_slot >= signing::kBip39MnemonicWordCount) {
        return false;
    }
    ...
    const char* prefix = signing::provisioning_flow_import_prefix(global_slot);
...
static bool make_import_candidate_area(lv_obj_t* parent)
{
    ...
    const size_t global_slot = signing::provisioning_flow_import_global_word_slot();
    const char* prefix = signing::provisioning_flow_import_prefix(global_slot);
    if (prefix[0] == '\0') {
    ...
    for (uint16_t word_index = 0; word_index < signing::kBip39WordCount; ++word_index) {
        const char* word = signing::bip39_english_word(word_index);
        if (!signing::provisioning_flow_word_starts_with_prefix(word, prefix)) {
            continue;
        }
        if (candidate_count >= signing::kBip39WordCount) {
            break;
        }
...
bool modal_draw_import_word_entry_panel(const char* notice)
{
    ...
    lv_obj_t* panel = drawing_surface_create_panel_locked(UiPanelKind::import_word_entry);
    if (panel == nullptr) {
        return false;
    }
    lv_obj_remove_flag(panel, LV_OBJ_FLAG_SCROLLABLE);
...
bool modal_draw_backup_phrase_display(const char* backup_phrase)
{
    ...
    lv_obj_t* panel = drawing_surface_create_panel_locked(UiPanelKind::backup_phrase_display);
    if (panel == nullptr) {
        return false;
    }
    lv_obj_remove_flag(panel, LV_OBJ_FLAG_SCROLLABLE);
    ...
    for (size_t index = 0; index < kBackupPhrasePrefixCellCount; ++index) {
        ...
        lv_label_set_text_static(prefix_label, signing::provisioning_flow_backup_phrase_prefix_cell(index));
        lv_label_set_long_mode(prefix_label, LV_LABEL_LONG_CLIP);
        lv_obj_set_width(prefix_label, kMnemonicWordPrefixWidth);
        lv_obj_set_style_text_align(prefix_label, LV_TEXT_ALIGN_LEFT, 0);
        lv_obj_set_style_text_font(prefix_label, &lv_font_montserrat_14, 0);
...
bool modal_draw_pin_setup_panel(const char* notice)
{
    ...
    lv_obj_t* panel = drawing_surface_create_panel_locked(UiPanelKind::pin_entry);
    if (panel == nullptr) {
        return false;
    }
    lv_obj_remove_flag(panel, LV_OBJ_FLAG_SCROLLABLE);
...
bool modal_draw_settings_menu_panel()
{
    ...
    lv_obj_t* panel = drawing_surface_create_panel_locked(UiPanelKind::settings_menu);
    if (panel == nullptr) {
        return false;
    }
    lv_obj_remove_flag(panel, LV_OBJ_FLAG_SCROLLABLE);
    ...
    signing::HumanApprovalInputMode human_approval_input_mode =
        signing::HumanApprovalInputMode::pin;
    const bool human_approval_setting_read_ok =
        signing::read_human_approval_input_mode(&human_approval_input_mode);
    ...
    if (!make_settings_menu_row(
            ...
            human_approval_setting_read_ok
                ? signing::human_approval_input_mode_label(human_approval_input_mode)
                : "ERR",
            0,
            SetupButtonKind::outlined_keypad,
            lv_color_hex(theme::kPrimary),
        ...
        !make_settings_menu_row(
            ...
            signing_mode_read_ok
                ? (signing_mode == AuthorizationMode::policy ? "POLICY" : "USER")
                : "ERR",
            1,
            SetupButtonKind::outlined_keypad,
            lv_color_hex(theme::kPrimary),
...
bool modal_draw_chain_settings_menu_panel()
{
    ...
    lv_obj_t* panel = drawing_surface_create_panel_locked(UiPanelKind::chain_settings_menu);
    if (panel == nullptr) {
        return false;
    }
    lv_obj_remove_flag(panel, LV_OBJ_FLAG_SCROLLABLE);
...
bool modal_draw_sui_settings_panel(const SuiSettingsViewModel& model)
{
    ...
    lv_obj_t* panel = drawing_surface_create_panel_locked(UiPanelKind::sui_settings);
    if (panel == nullptr) {
        return false;
    }
    lv_obj_remove_flag(panel, LV_OBJ_FLAG_SCROLLABLE);
...
bool modal_draw_error_recovery_panel(bool confirm)
{
    ...
    lv_obj_t* panel = drawing_surface_create_panel_locked(UiPanelKind::error_recovery);
    if (panel == nullptr) {
        return false;
    }
    lv_obj_remove_flag(panel, LV_OBJ_FLAG_SCROLLABLE);
    ...
    const signing::LocalPinAuthSnapshot& snapshot)
{
    ...
    if (snapshot.purpose == LocalPinAuthPurpose::settings_signing_mode) {
        return "Enter PIN to change signing mode.";
    }
    if (snapshot.purpose == LocalPinAuthPurpose::settings_policy_reset) {
        return "Enter PIN to reset policy.";
    }
    if (snapshot.purpose == LocalPinAuthPurpose::settings_sui_accept_gas_sponsor) {
        return "Enter PIN to change Sui gas sponsor.";
    }
    if (snapshot.purpose == LocalPinAuthPurpose::settings_sui_zklogin_clear) {
        return "Enter PIN to clear Sui zkLogin.";
    }
    if (snapshot.purpose == LocalPinAuthPurpose::settings_change_pin) {
        if (snapshot.stage == LocalPinAuthStage::new_pin_entry) {
            return "Enter new PIN.";
        }
        if (snapshot.stage == LocalPinAuthStage::repeat_pin_entry) {
...
static const char* local_pin_auth_title(const signing::LocalPinAuthSnapshot& snapshot)
{
    if (snapshot.processing) {
    ...
    TimeoutWindow timeout_window)
{
    ...
    lv_obj_t* panel = drawing_surface_create_panel_locked(UiPanelKind::policy_update_review);
    if (panel == nullptr) {
        return false;
    }
    lv_obj_remove_flag(panel, LV_OBJ_FLAG_SCROLLABLE);
    ...
    TimeoutWindow timeout_window)
{
    ...
    lv_obj_t* panel = drawing_surface_create_panel_locked(UiPanelKind::sui_zklogin_review);
    if (panel == nullptr) {
        return false;
    }
    lv_obj_remove_flag(panel, LV_OBJ_FLAG_SCROLLABLE);
    ...
    UserSigningReviewTimerState timer)
{
    if (model.title[0] == '\0' ||
        model.row_count == 0 ||
        model.row_count > kUserSigningReviewMaxRows) {
        return false;
    }
    ...
    lv_obj_t* panel = drawing_surface_create_panel_locked(UiPanelKind::user_signing_review);
    if (panel == nullptr) {
        return false;
    }
    lv_obj_remove_flag(panel, LV_OBJ_FLAG_SCROLLABLE);
    ...
    for (size_t index = 0; index < model.row_count; ++index) {
        const bool wrapped_value =
            model.rows[index].kind == UserSigningReviewRowKind::wrapped_value ||
            model.rows[index].kind == UserSigningReviewRowKind::section;
    ...
    const TimeoutWindow timeout_window =
        timer.available ? timer.display_window : kTimeoutWindowNone;
    if (!make_screen_bottom_timeout_timer_bar(
            panel,
            timeout_window,
            timer.display_tick,
...
bool modal_draw_user_signing_review_timer(UserSigningReviewTimerState timer)
{
    ...
    lv_obj_t* panel = drawing_surface_panel_locked();
    if (panel == nullptr ||
        drawing_surface_panel_kind_locked() != UiPanelKind::user_signing_review) {
        return false;
    }
    clear_screen_bottom_timeout_timer_bar_locked();
    const TimeoutWindow timeout_window =
...
bool modal_draw_local_pin_auth_panel(const char* notice)
{
    ...
    lv_obj_t* panel = drawing_surface_create_panel_locked(UiPanelKind::local_pin_auth);
    if (panel == nullptr) {
        return false;
    }
    lv_obj_remove_flag(panel, LV_OBJ_FLAG_SCROLLABLE);
