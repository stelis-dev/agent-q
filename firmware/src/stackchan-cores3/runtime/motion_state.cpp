...
std::atomic<int> g_posture_state{static_cast<int>(MotionPostureState::awake)};
...
MotionPostureState current_posture_state()
{
    return static_cast<MotionPostureState>(g_posture_state.load(std::memory_order_relaxed));
}

void move_to_posture(MotionPostureState state)
{
    if (state == MotionPostureState::rest) {
...
public:
    ...
    void _on_end(stackchan::Modifiable& stackchan) override
    {
        ...
        auto& motion = stackchan.motion();
        if (current_posture_state() == MotionPostureState::rest) {
            move_to_posture(MotionPostureState::rest);
        } else {
...
{
    switch (state) {
        case MotionFeedbackState::head_lift:
