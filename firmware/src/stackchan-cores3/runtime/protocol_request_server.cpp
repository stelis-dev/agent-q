#include "protocol_request_server.h"
...
using ResponseWriter = signing::ResponseWriter;
...
constexpr const char* kTag = "ProtocolRequestServer";
...
constexpr uint32_t kProtocolRequestTaskStackBytes = 24 * 1024;
...
using ProtocolTransport = signing::ProtocolTransport;

signing::ProtocolTransportRoute g_current_request_route = {};
signing::ProtocolTransportRoute g_pending_connect_route = {};
ProtocolTransport g_active_session_transport = ProtocolTransport::none;
signing::ProtocolTransportRoute g_active_user_signing_route = {};
signing::ProtocolTransportRoute g_pending_policy_update_route = {};
signing::ProtocolTransportRoute g_pending_credential_proposal_route = {};
signing::ProtocolTransportRoute g_signing_outcome_route_context = {};
...
const ResponseWriter& usb_operation_response_writer()
{
...
struct ScopedRequestRoute {
    explicit ScopedRequestRoute(const signing::ProtocolTransportRoute& next)
        : previous(g_current_request_route)
    {
        g_current_request_route = next;
    }

    ~ScopedRequestRoute()
    {
        g_current_request_route = previous;
    }

    signing::ProtocolTransportRoute previous;
};
...
{
    const signing::ProtocolTransportRoute route = g_pending_connect_route;
    g_pending_connect_route.clear();
    const bool written = route.response_writer()
                             .for_method("connect")
                             .write_error(id, code);
    return written;
}
...
bool write_connect_approved_response_for_pending_transport(const char* id)
{
    const signing::ProtocolTransportRoute route = g_pending_connect_route;
    g_pending_connect_route.clear();
    const bool written = write_connect_approved_with_writer(
        route.response_writer(),
        id);
    return written;
}

bool write_json_response_for_route(
    JsonDocument& response,
    const signing::ProtocolTransportRoute& route)
{
    return route.bound() &&
           signing::json_response_write(response, route.json_response_write_ops());
}

bool write_method_error_response_for_route(
    const char* id,
    const char* method,
    const char* code,
    const signing::ProtocolTransportRoute& route)
{
    return route.bound() &&
           route.response_writer().for_method(method).write_error(id, code);
}
...
bool write_method_error_response_for_active_user_signing(
    ...
    const char* code)
{
    return write_method_error_response_for_route(
        id,
        method,
        code,
        g_active_user_signing_route);
}

bool write_retry_response_json(JsonDocument& response, void*)
{
    return write_json_response_for_route(response, g_current_request_route);
}
    ...
    void* context)
{
    const signing::ProtocolTransportRoute route =
        context != nullptr
            ? *static_cast<const signing::ProtocolTransportRoute*>(context)
            : signing::ProtocolTransportRoute{};
    return write_method_error_response_for_route(id, method, code, route);
}

signing::SigningOutcomeWriterOps signing_outcome_writer_ops_for_route(
    const signing::ProtocolTransportRoute& route)
{
    g_signing_outcome_route_context = route;
    return signing::SigningOutcomeWriterOps{
        route.json_response_write_ops(),
        write_method_error_for_signing_outcome,
        &g_signing_outcome_route_context,
    };
}
...
{
    return signing::signing_outcome_write_policy_execution(
        id,
        signing::session_id(),
        request_identity,
        result,
        signing_outcome_writer_ops_for_route(g_current_request_route));
}
    ...
    const char* reason_code)
{
    const signing::PolicyUpdateFlowSnapshot snapshot =
        signing::policy_update_flow_snapshot();
    return signing::policy_propose_outcome_write(
        id,
        status,
        reason_code,
        &snapshot,
        g_pending_policy_update_route.response_writer());
}
...
ProtocolTransport current_protocol_state_owner_transport()
{
    if (g_active_session_transport != ProtocolTransport::none) {
        return g_active_session_transport;
    }
    if (g_pending_connect_route.bound()) {
        return g_pending_connect_route.transport();
    }
    if (g_pending_policy_update_route.bound()) {
        return g_pending_policy_update_route.transport();
    }
    if (g_pending_credential_proposal_route.bound()) {
        return g_pending_credential_proposal_route.transport();
    }
    return g_active_user_signing_route.transport();
}

void handle_persistent_material_consistency_error(const char* message)
{
    clear_session_bound_state_for_plan(
        current_protocol_transport_loss_plan(
            current_protocol_state_owner_transport(),
            xTaskGetTickCount()),
...
void finish_user_signing_error_terminal(
    ...
    const char* display_message)
{
    const signing::ProtocolTransportRoute response_route =
        g_active_user_signing_route;
    if (request_id != nullptr && request_id[0] != '\0') {
        const signing::UserSigningFlowCoreSnapshot snapshot =
            signing::user_signing_flow_core_snapshot();
        write_method_error_response_for_route(
            request_id,
            snapshot.method,
            error_code,
            response_route);
    ...
    g_active_user_signing_route.clear();
    ...
    const signing::UserSigningOutput* signing_output)
{
    ...
    if (result == signing::UserSigningTerminalResult::signed_success) {
        wrote_response = signing_output != nullptr &&
            signing::signing_outcome_write_user_signed(
                request_id,
                signing::session_id(),
                "user",
                snapshot,
                *signing_output,
                signing_outcome_writer_ops_for_route(response_route));
        ...
        wrote_response = signing::signing_outcome_write_user_terminal(
            request_id,
            signing::session_id(),
        ...
        wrote_response = write_method_error_response_for_route(
            request_id,
            snapshot.method,
            "invalid_session",
            response_route);
    ...
    if (!wrote_response) {
        if (response_route.response_writer().log_write_failure != nullptr) {
            response_route.response_writer().log_write_failure(
                response_type,
                request_id);
        }
    ...
    void* context)
{
    ...
    return signing::signing_outcome_user_signed_response_fits(
...
bool replace_active_session()
{
    ...
    g_active_session_transport = g_pending_connect_route.transport();
    return true;
}

signing::ProtocolPinLossPurpose protocol_pin_loss_purpose(
    const signing::ProtocolPinApprovalSnapshot& snapshot)
{
    if (!snapshot.active) {
        return signing::ProtocolPinLossPurpose::none;
    }
    switch (snapshot.purpose) {
        case signing::ProtocolPinApprovalPurpose::connect:
            return signing::ProtocolPinLossPurpose::connect;
        case signing::ProtocolPinApprovalPurpose::policy_update:
            return signing::ProtocolPinLossPurpose::policy_update;
        case signing::ProtocolPinApprovalPurpose::sui_zklogin_proposal:
            return signing::ProtocolPinLossPurpose::credential_proposal;
        case signing::ProtocolPinApprovalPurpose::none:
            return signing::ProtocolPinLossPurpose::other;
    }
    return signing::ProtocolPinLossPurpose::other;
}

signing::LocalPinLossPurpose local_pin_loss_purpose(
    const signing::LocalPinAuthSnapshot& snapshot)
{
    if (!snapshot.flow_active) {
        return signing::LocalPinLossPurpose::none;
    }
    switch (snapshot.purpose) {
        case LocalPinAuthPurpose::connect:
            return signing::LocalPinLossPurpose::connect;
        case LocalPinAuthPurpose::policy_update:
            return signing::LocalPinLossPurpose::policy_update;
        case LocalPinAuthPurpose::sui_zklogin_proposal:
            return signing::LocalPinLossPurpose::credential_proposal;
        ...
        case LocalPinAuthPurpose::settings_sui_zklogin_clear:
            return signing::LocalPinLossPurpose::other;
    }
    return signing::LocalPinLossPurpose::other;
}
    ...
    TickType_t now)
{
    ...
    state.lost_transport = lost_transport;
    state.session = {
    ...
    state.connect_approval = {
        signing::connect_approval_active(),
        g_pending_connect_route.transport(),
    };
    const ProtocolTransport session_bound_transport =
        g_active_session_transport != ProtocolTransport::none
            ? g_active_session_transport
            : g_pending_connect_route.transport();
    state.protocol_pin = protocol_pin_loss_purpose(protocol_pin);
    state.protocol_pin_transport = session_bound_transport;
    state.local_pin = local_pin_loss_purpose(pin_auth);
    state.local_pin_transport = session_bound_transport;
    state.policy_update = {
        signing::policy_update_flow_active(),
        g_pending_policy_update_route.transport(),
    };
    state.credential_proposal = {
        signing::sui_zklogin_proposal_flow_active(),
        g_pending_credential_proposal_route.transport(),
    };
    state.user_signing = {
        user_signing.active,
        g_active_user_signing_route.transport(),
    };
    ...
    const char* log_message)
{
    ...
    if (plan.clear_credential_review_panel) {
    ...
    switch (event) {
        ...
        case signing::UsbLinkEvent::unchanged_disconnected:
            ...
            if (grace == signing::UsbGraceAction::confirm) {
                if (current_protocol_transport_loss_plan(
                        ProtocolTransport::usb,
                        now).relevant) {
...
bool unrelated_user_signing_ingress_busy()
{
    const signing::OperationManifestEntry* entry =
        signing::operation_manifest_entry(
            signing::OperationType::sign_personal_message);
    if (entry == nullptr) {
        return true;
    }
...
{
    const signing::OperationManifestEntry* entry =
        signing::operation_manifest_entry(operation);
    if (entry == nullptr) {
    ...
    const ResponseWriter& writer)
{
    return write_busy_if_pending_or_local_flow_active_for_operation(
        id,
        writer,
        signing::OperationType::identify_device);
}
...
{
    if (request_id != nullptr && request_id[0] != '\0') {
        g_pending_credential_proposal_route.response_writer()
...
void sync_local_transport_pairing_panel()
{
    const signing::LocalTransportPairingSnapshot snapshot =
        signing::local_transport_pairing_snapshot();
    if (snapshot.active) {
        if (ui_panel_active(UiPanelKind::local_transport_pairing)) {
            return;
        }
        const signing::TimeoutWindow window =
            signing::timeout_window_from_deadline(
                xTaskGetTickCount(),
                snapshot.deadline);
        if (!modal_draw_local_transport_pairing_panel(
                snapshot.payload,
                snapshot.fingerprint_hex,
                window)) {
...
const signing::OperationHandlers& protocol_operation_handlers();

void handle_protocol_request_line(
    const char* line,
    const signing::ProtocolTransportRoute& route)
{
    ScopedRequestRoute scoped_route(route);
    signing::handle_protocol_request_line(
        line,
        current_timeout_tick(),
        route,
        protocol_operation_handlers(),
    ...
    signing::TimeoutWindow approval_window)
{
    ...
    g_pending_connect_route = g_current_request_route;
    return true;
}
...
void handle_connect_request(
    ...
    const ResponseWriter& writer)
{
    signing::handle_protocol_connect_request(id, request, writer, connect_handler_ops());
}
...
void handle_get_result_request(
    ...
    const ResponseWriter& writer)
{
    signing::handle_protocol_get_result_request(id, request, writer, retained_response_handler_ops());
}
...
void handle_ack_result_request(
    ...
    const ResponseWriter& writer)
{
    signing::handle_protocol_ack_result_request(id, request, writer, retained_response_handler_ops());
}
...
void handle_get_capabilities_request(
    ...
    const ResponseWriter& writer)
{
    signing::handle_protocol_get_capabilities_request(id, request, writer, session_read_handler_ops());
}
...
void handle_get_accounts_request(
    ...
    const ResponseWriter& writer)
{
    signing::handle_protocol_get_accounts_request(id, request, writer, session_read_handler_ops());
}
...
void handle_policy_get_request(
    ...
    const ResponseWriter& writer)
{
    signing::handle_protocol_policy_get_request(id, request, writer, policy_handler_ops());
}
...
void handle_disconnect_request(
    ...
    const ResponseWriter& writer)
{
    signing::handle_protocol_disconnect_request(id, request, writer, disconnect_handler_ops());
}
    ...
    signing::TimeoutWindow review_window)
{
    ...
    if (result == signing::PolicyUpdateFlowBeginResult::ok) {
        g_pending_policy_update_route = g_current_request_route;
    }
    return result;
}
...
void handle_policy_propose_request(
    ...
    const ResponseWriter& writer)
{
    signing::handle_protocol_policy_propose_request(id, request, writer, policy_handler_ops());
}

signing::SuiZkLoginCredentialPrepareResult prepare_sui_zklogin_credential_for_common(
    const char*,
    signing::SuiZkLoginCredentialPreparation* output)
{
    ...
    if (identity.kind != signing::SuiActiveIdentityKind::native ||
        ...
        return signing::SuiZkLoginCredentialPrepareResult::invalid_state;
    }
...
signing::SuiZkLoginCredentialProposalBeginResult begin_sui_zklogin_proposal_for_common(
    ...
    signing::TimeoutWindow request_window)
{
    ...
    if (result == signing::SuiZkLoginProposalBeginResult::ok) {
        g_pending_credential_proposal_route = g_current_request_route;
        return signing::SuiZkLoginCredentialProposalBeginResult::ok;
    }
    return signing::SuiZkLoginCredentialProposalBeginResult::invalid_proof;
}
...
{
    static const signing::ActiveSessionRequestGuardOps prepare_guard_ops = {
        provisioned_material_ready,
        nullptr,
        write_payload_delivery_safe_read_admission_error,
        require_active_matching_session,
    };
    static const signing::ActiveSessionRequestGuardOps propose_guard_ops = {
        provisioned_material_ready,
        nullptr,
        nullptr,
        require_active_matching_session,
    ...
    static const signing::SuiZkLoginCredentialHandlerOps ops = {
        ...
        prepare_guard_ops,
        propose_guard_ops,
        signing::SessionIdMode::optional_default_empty,
        signing::SessionIdMode::optional_default_empty,
...
signing::PolicySigningExecutionResult execute_policy_sign_transaction(
    ...
    const signing::UserSigningTransactionBeginInput& input)
{
    ...
    if (result == signing::UserSigningFlowBeginResult::ok) {
        g_active_user_signing_route = g_current_request_route;
    }
    return result;
}
...
const signing::SigningHandlerOps& signing_handler_ops()
{
    static const signing::SigningHandlerOps ops = {
        ...
        execute_policy_sign_transaction,
...
void handle_sign_transaction_request(
    ...
    const ResponseWriter& writer)
{
    signing::handle_protocol_sign_transaction_request(id, request, writer, signing_handler_ops());
}
...
void handle_sign_personal_message_request(
    ...
    const ResponseWriter& writer)
{
    signing::handle_protocol_sign_personal_message_request(id, request, writer, signing_handler_ops());
}
...
void run_protocol_request_server_maintenance_phase()
{
...
void run_protocol_request_server_connect_response_phase()
{
...
void protocol_request_task(void*)
{
    ...
    // sent (run_protocol_request_server_connect_response_phase) before new input is read
    ...
    while (true) {
        run_protocol_request_server_tick();
...
void init_protocol_request_server()
{
    ...
    signing::connect_approval_clear();
    signing::identification_display_clear();
    wipe_setup_scratch("protocol request server init");
    clear_storage_maintenance_flow("protocol request server init");
    ...
    g_usb_ready = init_usb_transport();
    if (g_protocol_task == nullptr) {
        BaseType_t task_created = xTaskCreatePinnedToCore(
            protocol_request_task,
            "protocol_req",
            kProtocolRequestTaskStackBytes,
            nullptr,
            4,
            &g_protocol_task,
            1);
        if (task_created != pdPASS) {
            ESP_LOGE(kTag, "Protocol request task start failed");
    ...
    ESP_LOGI(
        kTag,
        "Protocol request server ready; USB=%s",
        g_usb_ready ? "ready" : "unavailable");
}
