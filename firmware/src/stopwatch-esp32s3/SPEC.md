...
| QR and encrypted BLE local transport | O | From unlocked idle with no USB data link, KEYA creates a fresh 120-second QR payload and advertises the matching BLE service. Host USB SOF has transport priority: it blocks pairing entry and closes an advertising, handshaking, or established BLE carrier through the normal local-transport loss cleanup. Removing USB does not restart BLE; another KEYA action and fresh QR are required. A power-only cable without host SOF does not block BLE. Noise XX establishes only an encrypted carrier; the normal shared `connect` request and StopWatch `LINK?` approval still create the protocol session. KEYA cancels the pairing window. Touch has no pairing or approval authority. |
