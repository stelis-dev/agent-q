...
constexpr uint16_t kExternalPowerPresentMv = 4000;
constexpr uint32_t kPowerButtonHoldMs = 1000;
...
bool _power_button_hold_reported = false;
uint32_t _power_button_pressed_at_ms = 0;
bool _single_click_reset_known = false;
bool _single_click_reset_enabled = false;
...
void Hal::pmic_init()
{
    ...
    _power_button_hold_reported = false;
    _power_button_pressed_at_ms = 0;
    _single_click_reset_known = false;
    _single_click_reset_enabled = false;
    ...
    setPowerButtonSingleClickResetEnabled(false);
...
bool Hal::isBatteryCharging()
{
    ...
    if (chg_result != M5PM1_OK) {
        return false;
    }

    const bool charging_active = charge_status == 0;

    return isExternalPowerPresent() && charging_active;
}

bool Hal::isExternalPowerPresent()
{
    if (!_pm1) {
        return false;
    }
    ...
    return vin_mv > kExternalPowerPresentMv;
}

bool Hal::setPowerButtonSingleClickResetEnabled(bool enabled)
{
    if (!_pm1) {
        return false;
    }
    if (_single_click_reset_known && _single_click_reset_enabled == enabled) {
        return true;
    }

    const bool disable_single_click_reset = !enabled;
    if (_pm1->setSingleResetDisable(disable_single_click_reset) != M5PM1_OK) {
        return false;
    }
    _single_click_reset_known = true;
    _single_click_reset_enabled = enabled;
    return true;
}

Hal::PowerButtonEvent Hal::readPowerButtonEvent()
{
    if (!_pm1) {
    ...
    bool pressed       = false;
    if (_pm1->btnGetState(&pressed) != M5PM1_OK) {
        return PowerButtonEvent::none;
    }

    if (pressed) {
        if (!_power_button_pressed) {
            _power_button_pressed       = true;
            _power_button_hold_reported = false;
            _power_button_pressed_at_ms = now;
        }
        if (!_power_button_hold_reported && now - _power_button_pressed_at_ms >= kPowerButtonHoldMs) {
            _power_button_hold_reported = true;
        }
        return PowerButtonEvent::none;
    }

    const bool was_pressed = _power_button_pressed;
    const bool was_hold    = _power_button_hold_reported;
    _power_button_pressed = false;
    _power_button_hold_reported = false;
    ...
    if (was_pressed) {
        return PowerButtonEvent::shortClick;
    }
    return PowerButtonEvent::none;
}
