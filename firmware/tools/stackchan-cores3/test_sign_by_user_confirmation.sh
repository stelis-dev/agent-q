...
usage() {
  cat >&2 <<'EOF'
Usage: firmware/tools/stackchan-cores3/test_sign_by_user_confirmation.sh

Compiles the StackChan CoreS3 sign_by_user confirmation
...
cat >"${TMP_DIR}/sign_by_user_confirmation_test.cpp" <<'CPP'
#include <ctype.h>
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
...
void reset_all()
{
    ...
    if (agent_q::sign_by_user_flow_in_signing_critical_section()) {
        expect(agent_q::sign_by_user_flow_record_signing_failed() ==
                   agent_q::AgentQSignByUserTransitionResult::ok,
               "test cleanup closes critical flow");
    }
    if (agent_q::sign_by_user_flow_terminal_pending()) {
        agent_q::AgentQSignByUserTerminalResult terminal = {};
...
int main()
{
    using Confirm = agent_q::AgentQSignByUserConfirmationResult;
    using FlowStage = agent_q::AgentQSignByUserStage;
    using PinPurpose = agent_q::AgentQLocalPinAuthPurpose;
    using PinStage = agent_q::AgentQLocalPinAuthStage;

    {
        reset_all();
        expect(agent_q::sign_by_user_confirmation_accept_review_and_begin_pin(99, 200) ==
                   Confirm::inactive,
               "cannot start signing PIN without active request");
        expect(!agent_q::sign_by_user_confirmation_pin_active(),
...

    {
        ...
        expect(agent_q::sign_by_user_flow_snapshot().stage == FlowStage::reviewing,
...

    {
        ...
        expect(pin.flow_active && pin.purpose == PinPurpose::sign_by_user &&
                   pin.stage == PinStage::pin_entry,
               "signing PIN state is request-bound");
        expect(agent_q::sign_by_user_flow_snapshot().stage == FlowStage::pin_entry,
        ...
        expect(agent_q::sign_by_user_confirmation_mark_pin_verification_started() ==
                   Confirm::ok,
               "signing PIN submit pauses only PIN input deadline");
        expect(agent_q::sign_by_user_confirmation_complete_pin_verify_job_and_write_history(
        ...
        expect(agent_q::sign_by_user_flow_snapshot().terminal_result ==
                   agent_q::AgentQSignByUserTerminalResult::canceled,
        ...
        expect(agent_q::sign_by_user_confirmation_accept_review_and_begin_pin(99, 120) ==
                   Confirm::ok,
        ...
        expect(agent_q::sign_by_user_flow_snapshot().pin_input_deadline == 0 &&
                   agent_q::sign_by_user_flow_snapshot().request_deadline == 300,
...

    {
        ...
        expect(agent_q::sign_by_user_flow_snapshot().pin_input_deadline == 0 &&
                   agent_q::sign_by_user_flow_snapshot().request_deadline == 130,
        ...
        expect(agent_q::sign_by_user_flow_snapshot().terminal_result ==
                   agent_q::AgentQSignByUserTerminalResult::timed_out,
...

    {
        ...
        expect(agent_q::sign_by_user_flow_snapshot().request_deadline == 300 &&
                   agent_q::sign_by_user_flow_snapshot().pin_input_deadline == 170,
...

    {
        ...
        expect(agent_q::sign_by_user_flow_accept_review(99, 220) ==
                   agent_q::AgentQSignByUserTransitionResult::ok,
...

    {
        ...
        expect(agent_q::sign_by_user_flow_snapshot().terminal_result ==
                   agent_q::AgentQSignByUserTerminalResult::history_error,
...

    {
        ...
        expect(agent_q::sign_by_user_flow_snapshot().request_id[0] != '\0',
...

    {
        ...
        expect(agent_q::sign_by_user_confirmation_record_device_rejected() ==
                   Confirm::ok,
               "device rejection clears request");
        expect(!agent_q::sign_by_user_confirmation_pin_active(),
               "device rejection clears signing PIN");
        expect(agent_q::sign_by_user_flow_snapshot().terminal_result ==
                   agent_q::AgentQSignByUserTerminalResult::rejected,
    ...
    if (failures != 0) {
        fprintf(stderr, "%d sign_by_user confirmation test(s) failed\n", failures);
        return 1;
    }
    printf("sign_by_user confirmation tests passed\n");
    return 0;
}
CPP
...
"${CXX_BIN}" -std=c++17 -Wall -Wextra -Werror \
  ...
  -I"${COMMON_ROOT}" \
  -I"${COMMON_ROOT}/sui" \
  "${TMP_DIR}/sign_by_user_confirmation_test.cpp" \
  "${AGENT_Q_DIR}/agent_q_sign_by_user_confirmation.cpp" \
  ...
  -o "${TMP_DIR}/sign_by_user_confirmation_test"

"${TMP_DIR}/sign_by_user_confirmation_test"
