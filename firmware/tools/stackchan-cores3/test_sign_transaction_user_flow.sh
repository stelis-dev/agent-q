...
usage() {
  cat >&2 <<'EOF'
Usage: firmware/tools/stackchan-cores3/test_sign_transaction_user_flow.sh

Compiles the StackChan CoreS3 device-confirmed sign_transaction_user flow owner
...
cat >"${TMP_DIR}/sign_transaction_user_flow_test.cpp" <<'CPP'
#include <ctype.h>
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
    ...
    void*)
{
    ...
    if (g_history_write_clears_flow) {
        expect(agent_q::sign_transaction_user_flow_clear() ==
    ...
    if (g_history_write_rejects_flow) {
        expect(agent_q::sign_transaction_user_flow_record_device_rejected() ==
                   agent_q::AgentQSignTransactionUserTransitionResult::wrong_stage,
               "history writer cannot reclassify confirmed request as rejected");
    }
    return g_history_write_result;
}

agent_q::AgentQSignTransactionUserTransitionResult record_verified_pin_and_write_confirmation_history(
    agent_q::AgentQSignTransactionUserHistoryWriteFn write_fn,
    void* context,
    TickType_t now = 100)
{
    const agent_q::AgentQSignTransactionUserTransitionResult pause =
        agent_q::sign_transaction_user_flow_pause_pin_deadline();
    if (pause != agent_q::AgentQSignTransactionUserTransitionResult::ok) {
        return pause;
    }
    return agent_q::sign_transaction_user_flow_record_pin_verified_and_write_confirmation_history(
        now,
        write_fn,
        context);
}
...
{
    const std::vector<uint8_t>& payload = valid_payload();
    return agent_q::sign_transaction_user_flow_begin(
...
int main()
{
    using Begin = agent_q::AgentQSignTransactionUserFlowBeginResult;
    using Stage = agent_q::AgentQSignTransactionUserStage;
    using Transition = agent_q::AgentQSignTransactionUserTransitionResult;
    using Terminal = agent_q::AgentQSignTransactionUserTerminalResult;
    ...
    expect(agent_q::sign_transaction_user_flow_clear() == Transition::inactive,
           "clear on inactive flow reports inactive");
    expect(!agent_q::sign_transaction_user_flow_active(), "clear leaves flow inactive");
    expect(agent_q::sign_transaction_user_flow_validate_session() == SessionValidation::missing,
    ...
    expect(agent_q::sign_transaction_user_flow_validate_session() == SessionValidation::ok,
           "matching active session validates");
    expect(agent_q::sign_transaction_user_flow_session_matches(agent_q::session_id()),
           "session match helper recognizes owner session");
    agent_q::AgentQSignTransactionUserFlowSnapshot retained_snapshot = snapshot;
    expect(agent_q::sign_transaction_user_flow_clear() == Transition::ok,
    ...
    expect(!agent_q::sign_transaction_user_flow_active(), "pre-critical clear leaves flow inactive");
    expect(agent_q::sign_transaction_user_flow_begin(
               make_valid_input("req_signature_1", agent_q::session_id(), payload.data(), payload.size())) ==
               Begin::ok,
    ...
    snapshot = agent_q::sign_transaction_user_flow_snapshot();
    ...
    expect(agent_q::sign_transaction_user_flow_consume_signable_payload(copied.data(), copied.size(), &copied_size) ==
    ...
    expect(agent_q::sign_transaction_user_flow_record_timeout(99) == Transition::deadline_not_reached,
           "timeout before deadline is rejected");
    expect(agent_q::sign_transaction_user_flow_accept_review(99, 0) == Transition::invalid_deadline,
           "zero PIN deadline is rejected");
    snapshot = agent_q::sign_transaction_user_flow_snapshot();
    expect(snapshot.stage == Stage::reviewing, "invalid PIN deadline leaves review active");
    expect(agent_q::sign_transaction_user_flow_accept_review(99, 200) == Transition::ok,
    ...
    expect(agent_q::sign_transaction_user_flow_in_signing_critical_section(),
           "critical section helper is true");
    expect(agent_q::sign_transaction_user_flow_clear() == Transition::busy,
    ...
    expect(agent_q::sign_transaction_user_flow_complete_signed() == Transition::payload_not_consumed,
           "signed terminal requires one-shot payload handoff first");
    expect(agent_q::sign_transaction_user_flow_consume_signable_payload(copied.data(), copied.size() - 1, &copied_size) ==
    ...
    expect(agent_q::sign_transaction_user_flow_complete_signed() == Transition::ok,
           "signed terminal is recorded");
    expect(agent_q::sign_transaction_user_flow_terminal_pending(), "signed terminal is pending");
    Terminal terminal = Terminal::none;
    expect(agent_q::sign_transaction_user_flow_consume_terminal_result(&terminal) &&
               terminal == Terminal::signed_success,
           "signed terminal is one-shot consumable");
    expect(!agent_q::sign_transaction_user_flow_active(), "terminal consume clears state");
    expect(!agent_q::sign_transaction_user_flow_consume_terminal_result(&terminal),
           "terminal result cannot be consumed twice");
    expect(strcmp(agent_q::sign_transaction_user_flow_terminal_status(Terminal::signed_success), "signed") == 0 &&
               strcmp(agent_q::sign_transaction_user_flow_terminal_reason(Terminal::signed_success), "device_confirmed") == 0,
           "signed terminal mapping");

    agent_q::sign_transaction_user_flow_clear();
    expect(begin_valid_flow("req_reject"), "begin before reject");
    expect(agent_q::sign_transaction_user_flow_record_device_rejected() == Transition::ok,
    ...
    expect(strcmp(agent_q::sign_transaction_user_flow_terminal_status(Terminal::rejected), "user_rejected") == 0 &&
               strcmp(agent_q::sign_transaction_user_flow_terminal_reason(Terminal::rejected), "user_rejected") == 0,
           "rejected terminal mapping");

    agent_q::sign_transaction_user_flow_clear();
    expect(begin_valid_flow("req_timeout"), "begin before timeout");
    expect(agent_q::sign_transaction_user_flow_record_timeout(300) == Transition::ok,
    ...
    expect(strcmp(agent_q::sign_transaction_user_flow_terminal_status(Terminal::timed_out), "user_timed_out") == 0 &&
               strcmp(agent_q::sign_transaction_user_flow_terminal_reason(Terminal::timed_out), "user_timed_out") == 0,
    ...
    expect(strcmp(agent_q::sign_transaction_user_flow_terminal_status(Terminal::history_error), "") == 0 &&
               strcmp(agent_q::sign_transaction_user_flow_terminal_reason(Terminal::history_error), "") == 0,
           "history error is cleanup-only, not a sign_result status");

    agent_q::sign_transaction_user_flow_clear();
    expect(begin_valid_flow("req_disconnect"), "begin before disconnect");
    expect(agent_q::sign_transaction_user_flow_cancel_for_session_loss() == Transition::session_still_active,
    ...
    expect(agent_q::sign_transaction_user_flow_cancel_for_disconnect("session_aaaaaaaaaaaaaaaa") ==
               Transition::invalid_session,
           "mismatched disconnect does not cancel");
    expect(agent_q::sign_transaction_user_flow_cancel_for_disconnect(agent_q::session_id()) ==
               Transition::ok,
    ...
    expect(strcmp(agent_q::sign_transaction_user_flow_terminal_status(Terminal::canceled), "") == 0 &&
               strcmp(agent_q::sign_transaction_user_flow_terminal_reason(Terminal::canceled), "") == 0,
           "canceled is cleanup-only, not a sign_result status");

    agent_q::sign_transaction_user_flow_clear();
    expect(begin_valid_flow("req_review_ui_loss"), "begin before review UI loss");
    expect(agent_q::sign_transaction_user_flow_cancel_for_ui_loss() == Transition::ok,
    ...
    expect(agent_q::sign_transaction_user_flow_accept_review(99, 200) == Transition::ok, "review before signing failure");
    expect(record_verified_pin_and_write_confirmation_history(write_confirmation_history,
               nullptr) == Transition::ok,
           "pin verification writes history before signing failure");
    expect(agent_q::sign_transaction_user_flow_record_signing_failed() == Transition::ok,
    ...
    expect(strcmp(agent_q::sign_transaction_user_flow_terminal_status(Terminal::signing_failed), "signing_failed") == 0 &&
               strcmp(agent_q::sign_transaction_user_flow_terminal_reason(Terminal::signing_failed), "signing_failed") == 0,
    ...
    expect(agent_q::sign_transaction_user_flow_accept_review(100, 200) == Transition::deadline_expired,
    ...
    expect(agent_q::sign_transaction_user_flow_pause_pin_deadline() == Transition::ok,
    ...
    expect(agent_q::sign_transaction_user_flow_accept_review(99, 120) == Transition::ok,
    ...
    expect(snapshot.request_deadline == 130 &&
           ...
           "PIN verification pauses only local input deadline");
    expect(agent_q::sign_transaction_user_flow_deadline_reached(130),
    ...
    expect(agent_q::sign_transaction_user_flow_accept_review(99, 99) == Transition::deadline_expired,
    ...
    expect(!agent_q::sign_transaction_user_flow_active(), "stale session leaves flow inactive");
    ...
    agent_q::session_clear();
    expect(agent_q::sign_transaction_user_flow_accept_review(99, 200) == Transition::invalid_session,
    ...
    expect(agent_q::sign_transaction_user_flow_cancel_for_disconnect(critical_session_id) == Transition::busy,
           "matching disconnect cannot cancel signing critical section");
    expect(agent_q::sign_transaction_user_flow_cancel_for_session_loss() == Transition::busy,
    ...
    expect(!agent_q::sign_transaction_user_flow_active(), "digest failure leaves flow inactive");
    g_digest_result = true;

    agent_q::AgentQSignTransactionUserBeginInput invalid_deadline =
        make_valid_input("req_bad_deadline", agent_q::session_id(), payload.data(), payload.size());
    invalid_deadline.request_deadline = 0;
    expect(agent_q::sign_transaction_user_flow_begin(invalid_deadline) == Begin::invalid_deadline,
           "zero review deadline rejected");
    expect(!agent_q::sign_transaction_user_flow_active(), "invalid deadline leaves flow inactive");
    ...
    expect(!agent_q::sign_transaction_user_flow_active(), "invalid transaction leaves flow inactive");
    ...
    expect(!agent_q::sign_transaction_user_flow_active(), "stored account mismatch leaves flow inactive");
    reset_account_stub();
    ...
    expect(!agent_q::sign_transaction_user_flow_active(), "missing account leaves flow inactive");
    reset_account_stub();
    ...
    expect(!agent_q::sign_transaction_user_flow_active(), "gas owner mismatch leaves flow inactive");

    agent_q::AgentQSignTransactionUserBeginInput invalid_network =
        make_valid_input("req_bad_network", agent_q::session_id(), payload.data(), payload.size());
    invalid_network.network = "staging";
    expect(agent_q::sign_transaction_user_flow_begin(invalid_network) == Begin::invalid_network,
           "unsupported network rejected");

    char unterminated_chain[agent_q::kAgentQSignTransactionUserChainSize] = {};
    memset(unterminated_chain, 's', sizeof(unterminated_chain));
    agent_q::AgentQSignTransactionUserBeginInput unterminated_chain_input =
    ...
    expect(agent_q::sign_transaction_user_flow_begin(unterminated_chain_input) == Begin::invalid_argument,
           "unterminated chain is rejected before method validation");
    expect(!agent_q::sign_transaction_user_flow_active(), "unterminated chain leaves flow inactive");

    char unterminated_method[agent_q::kAgentQSignTransactionUserMethodSize] = {};
    memset(unterminated_method, 'm', sizeof(unterminated_method));
    agent_q::AgentQSignTransactionUserBeginInput unterminated_method_input =
    ...
    expect(agent_q::sign_transaction_user_flow_begin(unterminated_method_input) == Begin::invalid_argument,
           "unterminated method is rejected before method validation");
    expect(!agent_q::sign_transaction_user_flow_active(), "unterminated method leaves flow inactive");

    char unterminated_network[agent_q::kAgentQSignTransactionUserNetworkSize] = {};
    memset(unterminated_network, 'd', sizeof(unterminated_network));
    agent_q::AgentQSignTransactionUserBeginInput unterminated_network_input =
    ...
    expect(agent_q::sign_transaction_user_flow_begin(unterminated_network_input) == Begin::invalid_argument,
           "unterminated network is rejected before network validation");
    expect(!agent_q::sign_transaction_user_flow_active(), "unterminated network leaves flow inactive");

    agent_q::AgentQSignTransactionUserBeginInput unsupported_method =
    ...
    expect(agent_q::sign_transaction_user_flow_begin(unsupported_method) == Begin::unsupported_method,
           "unsupported method rejected");

    agent_q::AgentQSignTransactionUserBeginInput empty_payload =
        make_valid_input("req_empty_payload", agent_q::session_id(), payload.data(), 0);
    expect(agent_q::sign_transaction_user_flow_begin(empty_payload) == Begin::invalid_payload,
           "empty payload rejected");

    char overlong_request_id[agent_q::kAgentQSignTransactionUserIdSize + 4] = {};
    ...
    expect(!agent_q::sign_transaction_user_flow_active(), "overlong request id leaves flow inactive");

    if (failures != 0) {
        fprintf(stderr, "%d sign_transaction_user flow test(s) failed\n", failures);
        return 1;
    }
    printf("sign_transaction_user flow tests passed\n");
    return 0;
}
CPP
...
"${CXX_BIN}" -std=c++17 -Wall -Wextra -Werror \
  ...
  -I"${COMMON_ROOT}" \
  -I"${COMMON_ROOT}/sui" \
  "${TMP_DIR}/sign_transaction_user_flow_test.cpp" \
  ...
  -o "${TMP_DIR}/sign_transaction_user_flow_test"

"${TMP_DIR}/sign_transaction_user_flow_test"
