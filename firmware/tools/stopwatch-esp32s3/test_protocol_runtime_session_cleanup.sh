...
for required in \
  ...
  "${RUNTIME_DIR}/protocol_runtime.cpp" \
  ...
  "${COMMON_ROOT}/signing/sign_transaction_user_validation.cpp" \
	  ...
	  "${COMMON_ROOT}/signing/protocol_transport_loss.cpp" \
  ...
  "${COMMON_ROOT}/transport/connect_handler.cpp" \
  "${COMMON_ROOT}/transport/disconnect_handler.cpp" \
  "${COMMON_ROOT}/transport/payload_transfer_handlers.cpp" \
  "${COMMON_ROOT}/transport/retained_response_handlers.cpp" \
...
cat >"${TMP_DIR}/protocol_runtime_session_cleanup_test.cpp" <<'CPP'
...
int usb_serial_jtag_write_bytes(const void* buffer, size_t length, uint32_t)
{
    if (!g_usb_connected ||
        buffer == nullptr ||
        g_written_size + length >= sizeof(g_written)) {
        return -1;
    }
...
bool g_credential_preparation_active = false;
bool g_sui_zklogin_proposal_active = false;

void credential_preparation_state_clear()
{
    g_credential_preparation_clear_calls += 1;
    g_credential_preparation_active = false;
}
...
{
    CredentialPreparationSnapshot snapshot = {};
    snapshot.active = g_credential_preparation_active;
    return snapshot;
}
...
bool write_local_method_error(const char*, const char*, const char*)
{
    ++g_local_transport_write_calls;
    return true;
}

bool write_local_bytes(const char*, size_t, void*)
{
    ++g_local_transport_write_calls;
    return true;
}

void log_local_write_failure(const char*, const char*) {}

const signing::ProtocolTransportEndpoint& test_local_transport_endpoint()
{
    static const signing::ProtocolTransportEndpoint endpoint{
        signing::ProtocolTransport::local_transport,
        signing::ResponseWriter{
            write_local_method_error,
            log_local_write_failure,
        },
...
void replace_usb_session()
{
    assert(signing::session_replace(fill_random, nullptr) ==
           signing::SessionStartResult::ok);
    g_active_session_transport = signing::ProtocolTransport::usb;
}

void replace_local_transport_session()
{
    assert(signing::session_replace(fill_random, nullptr) ==
           signing::SessionStartResult::ok);
    g_active_session_transport = signing::ProtocolTransport::local_transport;
}

void own_pending_response_from_usb()
{
    g_pending_response_route = signing::ProtocolTransportRoute(
        usb_protocol_transport_endpoint());
}

void own_pending_response_from_local_transport()
{
    g_pending_response_route = signing::ProtocolTransportRoute(
        test_local_transport_endpoint());
}
...
void handle_test_request_line(const char* line)
{
    handle_protocol_request_line(
        line,
        signing::ProtocolTransportRoute(usb_protocol_transport_endpoint()));
...
void handle_test_operation(
    Handler handler,
    const char* id,
    ArduinoJson::JsonDocument& request)
{
    ScopedRequestContext request_context{
        signing::ProtocolTransportRoute(usb_protocol_transport_endpoint())};
    handler(id, request);
}
...
int main()
{
    ...
    reset_written();
    handle_test_operation(handle_get_accounts_request, "req_accounts_busy", accounts_while_busy);
    ...
    handle_test_operation(
        ...
        "req_connect_payload_busy",
        connect_while_payload_pending);
    assert(signing::session_active());
    ...
    handle_test_operation(
        handle_connect_request,
        "req_connect_payload_no_session_busy",
        connect_without_session_while_payload_pending);
    assert(!signing::session_active());
    ...
    handle_test_operation(
        ...
        "req_propose_payload_busy",
        propose_while_payload_pending);
    assert(signing::session_active());
    expect_written_contains("\"id\":\"req_propose_payload_busy\"");
    expect_written_contains("\"code\":\"busy\"");

    reset_written();
    handle_test_request_line(
    ...
    expect_written_contains("\"code\":\"busy\"");
    assert(!protocol_runtime_identification_display().active);
    ...
    handle_test_operation(
        ...
        "req_prepare_locked_no_payload",
        prepare_while_locked_without_payload);
    assert(signing::session_active());
    ...
    handle_test_operation(
        handle_connect_request,
        "req_connect_locked_session",
        connect_while_locked_with_session);
    assert(signing::session_active());
    ...
    handle_test_operation(
        handle_connect_request,
        "req_connect_credential_error",
        connect_credential_error_with_session);
    assert(!signing::session_active());
    ...
    handle_test_operation(
        handle_connect_request,
        "req_connect_auth_storage_error",
        connect_auth_storage_error_without_payload);
    assert(!signing::session_active());
    ...
    handle_test_operation(
        ...
        "req_disconnect_locked",
        disconnect_while_locked);
    assert(!signing::session_active());
    ...
    handle_test_operation(
        handle_connect_request,
        "req_connect_pending_busy",
        connect_while_pending_no_session);
    assert(!signing::session_active());
    assert(protocol_runtime_pending_request().kind == PendingRequestKind::connect);
    ...
    handle_test_operation(
        handle_connect_request,
        "req_connect_ui_busy",
        connect_while_ui_busy);
    assert(!signing::session_active());
    assert(g_pending_request.kind == PendingRequestKind::none);
    ...
    own_pending_response_from_usb();
    g_credential_preparation_active = true;
    g_sui_zklogin_proposal_active = true;
    ...
    reset_written();
    handle_test_operation(handle_disconnect_request, "req_disconnect", disconnect);
    ...
    handle_test_operation(
        handle_get_accounts_request,
        "req_accounts_after_session_loss",
        accounts_after_hidden_session_loss);
    assert(g_pending_request.kind == PendingRequestKind::none);
    ...
    g_now_ms = 900;
    g_usb_connected = true;
    assert(protocol_runtime_status().usb_connected);
    g_now_ms = 1000;
    g_usb_connected = false;
    const ProtocolStatus disconnected_status = protocol_runtime_status();
    assert(!disconnected_status.usb_connected);
    assert(g_pending_request.kind == PendingRequestKind::credential_propose);
    assert(signing::session_active());
    assert(g_credential_preparation_clear_calls == 0);
    assert(g_sui_zklogin_proposal_clear_calls == 0);
    g_now_ms = 3000;
    const ProtocolStatus confirmed_disconnected_status = protocol_runtime_status();
    assert(!confirmed_disconnected_status.usb_connected);
    assert(g_pending_request.kind == PendingRequestKind::none);
    assert(!signing::session_active());
    assert(g_credential_preparation_clear_calls > 0);
    assert(g_sui_zklogin_proposal_clear_calls > 0);
    ...
    replace_local_transport_session();
    g_pending_request.kind = PendingRequestKind::credential_propose;
    own_pending_response_from_local_transport();
    g_credential_preparation_active = true;
    g_sui_zklogin_proposal_active = true;
    snprintf(
        g_pending_request.id,
        sizeof(g_pending_request.id),
        "req_propose_local_during_usb_drop");
    g_credential_preparation_clear_calls = 0;
    g_sui_zklogin_proposal_clear_calls = 0;
    g_local_transport_write_calls = 0;
    reset_written();
    g_now_ms = 7000;
    g_usb_connected = true;
    assert(protocol_runtime_status().usb_connected);
    ...
    assert(!protocol_runtime_status().usb_connected);
    assert(signing::session_active());
    assert(g_active_session_transport == signing::ProtocolTransport::local_transport);
    assert(g_pending_request.kind == PendingRequestKind::credential_propose);
    assert(g_pending_response_route.transport() ==
           signing::ProtocolTransport::local_transport);
    assert(g_credential_preparation_clear_calls == 0);
    assert(g_sui_zklogin_proposal_clear_calls == 0);
    assert(g_local_transport_write_calls == 0);
    assert(g_written_size == 0);
    ...
    assert(!signing::session_active());
    assert(g_pending_request.kind == PendingRequestKind::none);
    assert(!g_pending_response_route.bound());
    assert(g_credential_preparation_clear_calls > 0);
    assert(g_sui_zklogin_proposal_clear_calls > 0);
    assert(g_local_transport_write_calls > 0);
    ...
    g_credential_preparation_active = true;
    g_sui_zklogin_proposal_active = true;
    const int preparation_clears_before_local_loss =
        g_credential_preparation_clear_calls;
    const int proposal_clears_before_local_loss =
        g_sui_zklogin_proposal_clear_calls;
    clear_protocol_state_after_transport_loss(
        signing::ProtocolTransport::local_transport);
    assert(signing::session_active());
    assert(g_active_session_transport == signing::ProtocolTransport::usb);
    assert(g_pending_request.kind == PendingRequestKind::credential_propose);
    assert(g_pending_response_route.transport() == signing::ProtocolTransport::usb);
    assert(g_credential_preparation_clear_calls ==
           preparation_clears_before_local_loss);
    assert(g_sui_zklogin_proposal_clear_calls ==
           proposal_clears_before_local_loss);
...
"${CXX_BIN}" -std=c++17 -Wall -Wextra -Werror -ffunction-sections -fdata-sections \
  ...
  "${TMP_DIR}/protocol_runtime_session_cleanup_test.cpp" \
  "${COMMON_ROOT}/protocol/session_state.cpp" \
  "${RUNTIME_DIR}/sensitive_memory.cpp" \
  "${RUNTIME_DIR}/state_projection.cpp" \
  "${COMMON_ROOT}/protocol/device_contract.cpp" \
  ...
  "${COMMON_ROOT}/protocol/device_handlers.cpp" \
  "${COMMON_ROOT}/protocol/json_response.cpp" \
  "${COMMON_ROOT}/protocol/operation_dispatch.cpp" \
  "${COMMON_ROOT}/protocol/operation_manifest.cpp" \
  "${COMMON_ROOT}/protocol/operation_type.cpp" \
  "${COMMON_ROOT}/protocol/request_envelope.cpp" \
  "${COMMON_ROOT}/protocol/request_line_handler.cpp" \
  "${COMMON_ROOT}/protocol/session_read_handlers.cpp" \
  "${COMMON_ROOT}/protocol/sui_zklogin_credential_handlers.cpp" \
  "${COMMON_ROOT}/policy/policy_handlers.cpp" \
  "${COMMON_ROOT}/signing/policy_signing_execution_result.cpp" \
  "${COMMON_ROOT}/signing/sign_personal_message_user_ingress.cpp" \
  ...
  -Wl,-dead_strip \
  -o "${TMP_DIR}/protocol_runtime_session_cleanup_test"

"${TMP_DIR}/protocol_runtime_session_cleanup_test"
echo "StopWatch protocol runtime session cleanup tests passed"
