...
| Policy update | X | Not implemented. The future contract is specified as a Firmware-owned `propose_policy_update` flow with bounded proposal validation, device-local approval, rollback-safe canonical storage, and no direct policy setter. This target now has a target-local JSON policy-proposal parser/validator source that bounds the serialized policy object after deserialization for the future flow, but it still has no raw protocol envelope handler, pending-update state, approval UI, custom policy storage, or Gateway/Admin surface. |
