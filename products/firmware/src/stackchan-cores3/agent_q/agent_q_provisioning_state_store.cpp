...
constexpr const char* kTag = "AgentQProvState";
...
bool provisioning_state_store_load(AgentQProvisioningStateStoreRecord* output)
{
    ...
    output->status = AgentQProvisioningStateStorageStatus::unreadable;
    output->value[0] = '\0';
    ...
    if (result == ESP_ERR_NVS_NOT_FOUND) {
        output->status = AgentQProvisioningStateStorageStatus::missing;
        output->value[0] = '\0';
        return true;
    }
    if (result == ESP_OK) {
        output->status = AgentQProvisioningStateStorageStatus::present;
        return true;
    }
