...
| `connect` | forbidden | required | `ConnectPayload` object: `{ clientName }` | `ConnectResult` object: session id, session TTL, and device status | Firmware session admission and recovery gate. If the current USB physical link already has an approved live Firmware session, sessionless `connect` returns that existing session without starting a new approval and without replacing the session. Otherwise, admission creates a normal material-backed session or a limited target-owned material-install bootstrap session after Firmware-owned local authentication, target-defined policy requirements, and device-local approval. Firmware returns `invalid_state` when its current session policy refuses both session recovery and new admission. |
| `disconnect` | required | forbidden | no payload | `DisconnectResult` object: `{}` | Active session cleanup gate. |
| `get_capabilities` | required | forbidden | no payload | `CapabilitiesResult` object: chains, accounts, methods, signing state, and credential support | Active session and target capability state gate. Missing or unavailable target-owned material must be reflected through shared capability data and shared errors, not a target-specific schema. |
| `get_accounts` | required | forbidden | no payload | `AccountsResult` object: public accounts | Active session and account availability gate. Missing or unavailable account material must be reflected through the shared accounts result or a shared error, not a target-specific schema. |
