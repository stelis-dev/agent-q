...
usage() {
  cat >&2 <<'EOF'
Usage: tools/firmware/common/test_sui_transaction_facts.sh
...
for required in \
  ...
  if [[ ! -f "${required}" ]]; then
    ...
    echo "Run tools/firmware/common/generate_sui_transaction_fixtures.mjs first if fixtures are missing." >&2
    exit 1
  fi
done
