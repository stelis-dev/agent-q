...
usage() {
  cat >&2 <<'EOF'
Usage: tools/firmware/stackchan-cores3/test_connect_approval.sh
...
int main()
{
    ...
    expect(agent_q::connect_approval_begin("connect-1", "Agent-Q Gateway", 100),
    ...
    expect(snapshot.deadline == 100, "deadline stored");
    ...
    expect(!agent_q::connect_approval_begin("connect-2", "Other Gateway", 125),
    ...
    expect(!agent_q::connect_approval_begin(too_long_id, "Agent-Q Gateway", 200),
    ...
    expect(!agent_q::connect_approval_begin("connect-3", too_long_gateway, 200),
           "overlong gateway name is rejected");
    expect(!agent_q::connect_approval_active(), "overlong gateway name does not activate state");
    expect(!agent_q::connect_approval_begin("connect-3", "", 200),
           "empty gateway name is rejected");
    expect(!agent_q::connect_approval_begin("", "Agent-Q Gateway", 200),
           "empty request id is rejected");

    expect(agent_q::connect_approval_begin("connect-4", "Agent-Q Gateway", 300),
