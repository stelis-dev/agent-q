...
usage() {
  cat >&2 <<'EOF'
Usage: tools/firmware/stackchan-cores3/test_prepare_sync.sh
...
if [[ ! -f "${FIRMWARE_DIR}/main/CMakeLists.txt" ||
  ...
  echo "Run tools/firmware/stackchan-cores3/build.sh first, or set AGENT_Q_STACKCHAN_FIRMWARE_DIR." >&2
  rm -rf "${TMP_DIR}"
  exit 1
fi
